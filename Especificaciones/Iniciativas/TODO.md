...
1. **`I-17` BD de dos niveles está COMPLETA local (6/6 slices, 2026-07-22, 420 pruebas verdes).** El objetivo pasa a **rotar al siguiente ítem de §4 / `00_Indice §2`**. Orden de la tabla tras I-17: `I-12` seed thoughts (**BLOCKED** — seeds de Felipe no recibidos; escalar, no implementar), `I-13` decisión agnóstica-vs-tailored (**espera decisión GHT 25-jul**; no codificar hasta tenerla), `I-14` **tags** (TODO, primer accionable sin bloqueo externo). **Primer paso ejecutable = leer la spec de `I-14` + `00_Indice §2/§3` y confirmar con el usuario cualquier decisión de diseño antes de codificar.** Alternativas que el usuario puede priorizar: (a) commitear/push de I-17 (un push despliega a producción); (b) activar el runbook I-01 (calibrar umbral 0.6 con D5 real + fijar flags globales `CierreAnticipadoHabilitado`/`Parafraseo`/`MinutosInactividadSesion=5` en el acta del día-D). `P-14`/`I-10` no son objetivo. **Regla vigente:** ante una decisión de diseño real de la nueva iniciativa, confírmala con el usuario antes de implementar.
