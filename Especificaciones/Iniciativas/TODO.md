...
> **ESTADO VIGENTE 2026-07-25 — pista de auditoría: `P-19` COMPLETO local; siguiente `P-20`:** `P-15` a **`P-19`** permanecen DONE. `P-19` (`UXA11Y-002`) añadió regiones vivas persistentes para errores, éxito e información, avisos globales sin anuncios duplicados y asociaciones de error a campos activos. No cambian textos, validación, APIs, permisos ni acciones. Se verificó con Node temporal 24.15.0 y Angular local: formato, build y **18 pruebas frontend verdes**. **Próximo ejecutable: `P-20`** (`UXA11Y-003`). `I-12` continúa bloqueada por seeds, `I-13` espera decisión GHT e `I-14` sigue bloqueada por el catálogo consolidado de etiquetas: **no crear, inferir ni hardcodear etiquetas**.
...
| **15h** | **`P-20` pestañas accesibles de campañas** | **Remediación auditoría** | **Claude** | **NEXT — después de P-19 y P-16.** `UXA11Y-003`: patrón ARIA completo, foco móvil y teclado en el detalle de campaña. Spec `P-20_Pestanas_Accesibles_Campanias.md`. |
...
1. **Ejecutar P-20 — pestañas accesibles de campañas (`UXA11Y-003`).** P-16 ya dejó los paneles separados y P-19 ya cubre los anuncios dinámicos. En este entorno la vía verificada para Angular es Node temporal 24.15.0 (`npx --yes --package=node@24.15.0 -c "node .\\node_modules\\@angular\\cli\\bin\\ng.js <comando>"`) con los ejecutables locales: build y test son reproducibles. Leer P-20, trazar los selectores del detalle de campaña, completar el patrón ARIA con foco y teclado sin rediseñar, cubrir regresión y repetir build/test/format. `I-12`, `I-13` e `I-14` mantienen sus bloqueos externos: no crear, inferir ni hardcodear etiquetas mientras falte el catálogo GHT.
