...
**Iniciativa objetivo vigente: ninguna pendiente de implementación; `P-30` quedó COMPLETA local (3/3).**
El siguiente agente coordina la validación operativa transversal o espera la priorización de un nuevo
requisito, sin desplegar, hacer push ni modificar configuración remota.
...
| 36 | **`P-30` retomar ideas del pasado** | **DONE local 2026-08-04 (3/3)** | **Codex** | Selector histórico determinista por participante, campaña y pregunta, sin filtro por estado/ciclo; selección por número o título/resumen exacto, misma idea y conversación reabiertas, curaduría suspendida, kill-switch OFF, Cosmos, telemetría sin texto, E2E y QAS. Backend 729/729, build/format/diff verdes. **No hay siguiente requisito de código priorizado.** |
| DT-P27-01 | **Configuración versionada de expresiones determinísticas P-27** | **EN CURSO — 1/2 DONE local 2026-08-05** | Codex | Corte 1: lectura desde config, fallback a los defaults compilados y normalización compartida, backend 730/730. Corte 2 pendiente: validar vacíos/duplicados/límite, descartar con registro seguro e implementar historial/rollback. No permitir edición por campaña, no modificar alias ni activar P-27. Spec: `Iniciativas/DT-P27-01_Config_Versionada_Frases_Finalizacion.md`. |
...
1. **Implementado localmente `DT-QA-01` — endpoint de inyección de webhook simulado.** El camino de
   diagnóstico requiere `X-Diag-Key` fuera de Development, encola como un webhook firmado y no usa el
   App Secret de Meta. Queda pendiente solamente el despliegue controlado para E2E Azure; no cambiar
   configuración remota sin orden.

2. **Siguiente: `DT-P27-01` corte 2 de 2 — validación, observabilidad e historial/rollback.** Validar cada
   lista tras normalizar (vacíos, duplicados, límite); ante invalidez usar el default y registrar solo el
   motivo; completar historial/rollback y cerrar `Reglas`, QAS, `TODO.md` y `AVANCES.md`. No tocar alias
   vigentes ni activar P-27.

3. **En paralelo (operativo, no de código):** validación D5 real, UAT, costo/latencia y acta de flags
