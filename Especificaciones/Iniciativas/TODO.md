...
**Vas a implementar la iniciativa objetivo de este TODO: `ID-INICIATIVA=I-10` (flag de base previa vs. blanco). AGENTE ASIGNADO: `Codex`. VENTANA: Sprint 2.** No es un desarrollo desde cero: continúas un MVP vivo. **Contexto ya HECHO y verificado:** P-03, P-10, D5 (baseline real pendiente), I-16, I-08 backend **e I-08 UI (2026-07-20, Claude Sonnet 5)**, I-06 diseño+implementación local, I-09 diseño **e I-09 core**, I-05 parafraseo DONE local e I-03 follow-ups eje débil DONE local. **P-13 umbral de cierre por campaña DONE local (2026-07-21):** `configConversacional.umbralCierreAnticipado` nullable, default numérico `Conversacion:UmbralCierreAnticipado`, kill-switch real `Conversacion:CierreAnticipadoHabilitado` (default `true`), API/Cosmos/portal/telemetría y 400 pruebas backend verdes; D5 real y calibración I-01 en staging siguen pendientes. I-01 queda BLOCKED para activación real. **Tu objetivo ahora es I-10:** I-09 ya implementó el flag por campaña y el orquestador; el trabajo pendiente es exponer la activación explícita en el portal y alinear la spec que aún describe el campo como inexistente. Lee la spec, `03`/`04` actuales y el código antes de editar; confirma con el usuario cualquier decisión de diseño real antes de codificar. Al terminar, rota este TODO al siguiente ítem según §4 y actualiza siempre cabecera, tabla, §8 y AVANCES.md.
...
| 15 | `I-10` flag base previa/blanco | Sprint 2 | Codex | **← ACTUAL (objetivo de este TODO)** — I-09 core ya implementó `tejidoColectivo` y backend; falta checkbox explícito en portal + alinear spec/pruebas |
...
1. Identifica la iniciativa objetivo: **`I-10` flag de base previa vs. blanco** — agente **Codex**, Sprint 2. Leer `AVANCES.md`, `00_Indice_y_Plan_de_Ejecucion.md` (§2/§3), `Iniciativas/I-10_*.md`, `03`/`04` y el flujo I-09 ya implementado. **Estado real:** `configConversacional.tejidoColectivo`, Cosmos/API y el orquestador ya existen; la spec está desactualizada en ese punto. El primer paso ejecutable es definir el cambio mínimo de portal para que el admin active/desactive explícitamente el tejido por campaña, preservar los demás campos conversacionales en el `PUT` y añadir su prueba. Si aparece una decisión de diseño real, confírmala con el usuario antes de codificar, y rota el handoff al siguiente ítem ejecutable al cerrar.
