...
> **ESTADO VIGENTE 2026-07-25 — `P-21` multi-número de WhatsApp COMPLETA local:** configuración de números por alias compatible con el `PhoneNumberId` legado, captura de `metadata.phone_number_id`, respuesta siempre por el número entrante y alias opcional por campaña para el envío inicial. Sin secretos nuevos; alias vacío o inválido usa el predeterminado. Build Release sin warnings y **473 pruebas backend verdes** (420 unitarias + 53 integración). **PRÓXIMO OBJETIVO = `P-22` UX de Campañas**; P-23 queda después. `I-12`, `I-13` e `I-14` siguen bloqueadas por terceros y no se crean ni infieren etiquetas.
...
| 25 | **`P-22` UX de Campañas** | A coordinar (mejoras de portal) | por asignar | **TODO — spec lista 2026-07-25, implementación pendiente.** **Frontend-only, sin contratos.** Navegación guiada por ciclo de vida (pestañas como pasos numerados con completitud ✓/⚠), "Nueva campaña" como acción, enlace contextual "Ver envíos" (id real), Configuración en fieldsets con instrucciones y espaciado, estados vacíos. Preserva P-16/P-18/P-19/P-20. Spec `P-22_UX_Campanias.md`, `SUPUESTOS.md#ux-portal-campanias-resultados-p22-p23`. **Ejecutable ya**; verificar frontend con Node 24.15.0. |
| 26 | **`P-23` UX de Resultados** | A coordinar (mejoras de portal) | por asignar | **TODO — spec lista 2026-07-25, implementación pendiente.** **Frontend-only, sin contratos.** Selección de campaña sin fricción, patrón maestro-detalle (respuesta → evaluación + Markdown), leyenda de badges + conteos, lectura fácil, estados vacíos. Preserva I-17/P-18/P-19. Spec `P-23_UX_Resultados.md`, mismo supuesto que P-22. **Ejecutable ya**; verificar frontend con Node 24.15.0. |
...
1. **Siguiente trabajo ejecutable: `P-22` UX de Campañas (§4 fila 25).** Es frontend-only, sin contratos y preserva P-16/P-18/P-19/P-20. Antes de codificar, leer `Iniciativas/P-22_UX_Campanias.md` (§3 y §8) y `SUPUESTOS.md#ux-portal-campanias-resultados-p22-p23`; verificar con Node 24.15.0. Al cerrarlo, el handoff pasa a `P-23` UX de Resultados. Los ítems ordinarios siguen bloqueados por terceros: seeds de Felipe para `I-12`; decisión GHT agnóstica-vs-tailored para `I-13`; catálogo oficial de etiquetas para `I-14` (no crear, inferir ni hardcodear etiquetas).
