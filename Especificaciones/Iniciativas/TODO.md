...
**Vas a implementar la iniciativa objetivo de este TODO: `ID-INICIATIVA=I-09` (tejido colectivo — diseño). AGENTE ASIGNADO: `Codex`.** No es un desarrollo desde cero: continúas un MVP vivo. **Contexto (Sprint 1a, ya HECHO y verificado):** `P-03` reinicio de datos (`SUPUESTOS.md#reinicio-datos`), `P-10` completo — cupos (`#guardrails-cupos-conversacion`), rate por número (`#rate-numero-entrante`) y costo LLM por campaña (`#cupo-costo-llm`), **`D5` banco de calibración de prompts** (backend/tooling verde en CI; librería `src/ElTejido.Calibracion` + golden set `tests/Calibracion/golden-set.json` de 24 entradas sin PII + runner opt-in `[Trait("Category","Calibracion")]` fuera de CI; `SUPUESTOS.md#banco-calibracion-d5`; **baseline pendiente de un corrido real contra staging**, ver `tests/Calibracion/README.md`), **`I-16` fix de calificación en Markdown** (Markdown toma la evaluación más reciente por `fecha DESC`, regresión determinística verde), **`I-08` carga masiva de participantes — backend** (contrato aditivo `04 §5.1` + endpoint CSV; `SUPUESTOS.md#carga-masiva-participantes`; verde 335) e **`I-06` multi-idea — diseño** (contratos aditivos `03 §3.3/§3.8`, `04 §5.3/§5.8`, `ISegmentadorIdeas`, fallback 1-idea, rollback y métricas; `SUPUESTOS.md#multi-idea-i06-diseno`). **Tu objetivo ahora es `I-09` (diseño)** (spec `Especificaciones/Iniciativas/I-09_Tejido_Colectivo.md`): producir el **diseño técnico** de tejido colectivo/base previa (no implementación core todavía), respetando contratos y dependencias duras (§3). Si el diseño plantea decisiones A/B/C o toca contratos, **confírmalas con el usuario antes de cerrarlas**. **Rotación de agentes: al terminar I-09 diseño, deja este TODO listo para el siguiente ítem según §4 (`I-01` activar umbral en staging → Claude), siguiendo el orden y la alternancia Codex/Claude.**
...
| 7 | **`I-09` tejido colectivo (diseño)** | Sprint 1a | **Codex** | **← ACTUAL (objetivo de este TODO)** |
...
1. Identifica la iniciativa objetivo: **`I-09` (tejido colectivo — diseño)** — agente **Codex**. `P-03`, `P-10`, **`D5`**, **`I-16`**, **`I-08` backend** e **`I-06` diseño** ya están HECHOS (D5 verificado backend/tooling; baseline pendiente de un corrido real contra staging — ver `AVANCES.md`, `SUPUESTOS.md#banco-calibracion-d5`/`#carga-masiva-participantes`/`#multi-idea-i06-diseno`, tabla §4). Lee la spec `I-09_Tejido_Colectivo.md`; es un **ítem de diseño**: produce el diseño técnico de la funcionalidad de tejido colectivo/base previa, sin implementar todavía el core de recuperación/inyección. Documenta el diseño donde corresponda y **confirma con el usuario cualquier decisión A/B/C o cambio de contrato antes de cerrarla**. Respeta las dependencias duras (§3). **Al terminar, reescribe la cabecera y §8 de este TODO para apuntar al ítem 8 (`I-01` activar umbral en staging, agente Claude) y haz el handoff (§5–§6).**
