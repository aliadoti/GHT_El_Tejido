...
| **15b** | **`I-17` BD de dos niveles (maduro/incubación)** | **Sprint 1b–2** | **← ACTUAL (nuevo objetivo, reunión 20-jul)** | **BLOCKED-por-decisión** — spec creada; requiere **confirmar con el usuario los puntos I-17 §5** (¿umbral único o dos?, default 60/80, "guardar salvo no") ANTES de codificar. Reutiliza el patrón de umbral por campaña de P-13. |
...
1. Identifica la iniciativa objetivo: **`I-17` BD de dos niveles (ideas maduras vs. incubación)** — Sprint 1b–2. **`I-10` quedó DIFERIDA con I-09 (tejido colectivo) por la reunión del 20-jul; NO es el objetivo.** **Antes de codificar I-17 hay un bloqueo por decisión:** su spec (`Iniciativas/I-17_BD_Dos_Niveles_Madurez.md §5`) tiene tres puntos abiertos que **debes confirmar con el usuario** — (a) ¿el umbral de madurez es el **mismo** valor que `UmbralCierreAnticipado` (I-01/P-13) o uno independiente `UmbralMadurez`?, (b) default 60 % vs 80 %, (c) semántica de "guardar salvo que el usuario diga no". Además queda por decidir si el **cierre por inactividad ~5 min** (ítem 23) entra en I-17 §7 o va aparte. Solo tras esas confirmaciones: leer `AVANCES.md`, `00_Indice` (§2/§3), la spec I-17, `03 §3.8/§3.3`, `04 §5.8`, `05 §4.4`, `08`, `09` y el patrón de umbral por campaña ya implementado en P-13; luego implementar la clasificación determinista aditiva sobre `responses` (paráfrasis I-05 solo tras umbral) en pasos pequeños con pruebas.
