...
> **ESTADO VIGENTE 2026-07-24 — `ID-INICIATIVA=P-16`, TODO — PRÓXIMO EJECUTABLE:** la pista de remediación de auditoría avanza. **`P-15` COMPLETO (3/3 cortes DONE local):** el orquestador quedó como coordinadora de tres colaboradores — `PoliticaLimitesConversacion` (política sin E/S, Corte 1, commit `56bbef2`), `ResolvedorTransicionConversacion` (transición, Corte 2, commit `3e5cc9a`) y `ProcesadorResultadoEvaluacion` (efectos posteriores, Corte 3, sin commit aún); backend verde **468** (416 unit + 52 integration; +45 sobre 423), format limpio, sin cambio de comportamiento ni contratos. **Próximo = `P-16` (agente Claude): descomponer la página de campañas** (`CAL-002`) en componentes con entradas/salidas, ruta/API/permisos intactos — trabajo frontend Angular. Luego P-17→P-18→P-19→P-20 (P-20 depende de P-16). `I-12` continúa bloqueada por seeds, `I-13` espera decisión GHT e `I-14` sigue bloqueada por el catálogo consolidado de etiquetas: **no crear, inferir ni hardcodear etiquetas**.
...
1. **Ejecutar `P-16` — descomponer la página de campañas (`CAL-002`).** `P-15` quedó COMPLETO (3/3 cortes DONE local, backend verde 468). Parte de `P-16_Refactor_Pagina_Campanias.md`: separar listado, formulario, mensajes, preguntas, participantes y configuración de la página de campañas en componentes con entradas/salidas, manteniendo **ruta, API y permisos intactos** y sin cambio de comportamiento. Es trabajo **frontend Angular** — en este entorno `ng build`/`ng test` están bloqueados por el mismatch `@esbuild` bajo WSL/OneDrive; valida con `prettier --check` y `tsc --noEmit` (el CD reconstruye `wwwroot` en Linux). Al cerrarlo, rota a `P-17`. `I-12`, `I-13` e `I-14` mantienen sus bloqueos externos: no crear, inferir ni hardcodear etiquetas mientras falte el catálogo GHT.
