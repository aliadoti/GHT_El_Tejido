...
> **ESTADO VIGENTE 2026-07-24 — pista de auditoría: `P-16` COMPLETO local; siguiente `P-18`:** `P-15` y `P-17` permanecen DONE. **`P-16` (`CAL-002`) quedó DONE local:** `CampaniasPage` conserva carga, campaña seleccionada, permisos y llamadas; listado, creación, configuración, mensajes, preguntas, participantes/vista previa/reinicio y coordinación del detalle se separaron en paneles standalone con entradas/salidas tipadas. No cambian ruta, DTO, APIs ni autorización. Se verificó con Node temporal 24.15.0 usando los ejecutables Angular locales: build y **13 pruebas frontend verdes**. El supuesto bloqueo por `@esbuild` no se reprodujo con esa vía. **Próximo ejecutable: `P-18`** (`UXA11Y-001`), seguido por P-19 y P-20. `I-12` continúa bloqueada por seeds, `I-13` espera decisión GHT e `I-14` sigue bloqueada por el catálogo consolidado de etiquetas: **no crear, inferir ni hardcodear etiquetas**.
...
| **15f** | **`P-18` nombre accesible de controles** | **Remediación auditoría** | **Codex** | **NEXT — después de P-16.** `UXA11Y-001`: selección de envíos, tags y CSV con etiqueta o nombre ARIA contextual. Spec `P-18_Controles_Con_Nombre_Accesible.md`. |
...
1. **Ejecutar P-18 — controles con nombre accesible (`UXA11Y-001`).** P-16 ya está cerrada y deja separados los paneles de campañas que P-20 usará después. En este entorno la vía verificada para Angular es Node temporal 24.15.0 (`npx --yes --package=node@24.15.0 -c "node .\\node_modules\\@angular\\cli\\bin\\ng.js <comando>"`) con los ejecutables locales: build y test sí son reproducibles. Leer P-18, trazar selección de envíos/tags/CSV, agregar nombres accesibles sin rediseñar, cubrir regresión y repetir build/test/format. `I-12`, `I-13` e `I-14` mantienen sus bloqueos externos: no crear, inferir ni hardcodear etiquetas mientras falte el catálogo GHT.
