...
> **ESTADO VIGENTE 2026-07-24 — `ID-INICIATIVA=P-15`, Corte 3/3 — PRÓXIMO EJECUTABLE:** por decisión del usuario se abre una pista inmediata de remediación de auditoría. `P-15` corrige la cohesión interna del orquestador sin alterar su contrato ni el flujo conversacional, en 3 cortes. **Corte 1/3 DONE (commit `56bbef2`):** `PoliticaLimitesConversacion` (política determinista sin E/S: umbral/madurez/corte/elegibilidad de mejora). **Corte 2/3 DONE local 2026-07-24 (Claude Opus 4.8):** `ResolvedorTransicionConversacion` (interpretación de la situación + decisión de cerrar-sin-evaluar/evaluar-techos, con los detectores de intención dentro); backend verde **462** (410 unit + 52 integration; +19), format limpio; sin commit aún. **Próximo = Corte 3/3 `ProcesadorResultadoEvaluacion`** (efectos posteriores a la evaluación: persistencia, Markdown, mensajes y logs, conservando el orden). Al cerrar los 3, rotar a `P-16`. `I-12` continúa bloqueada por seeds, `I-13` espera decisión GHT e `I-14` sigue bloqueada por el catálogo consolidado de etiquetas: **no crear, inferir ni hardcodear etiquetas**.
...
| **15c** | **`P-15` refactor del orquestador conversacional** | **Remediación auditoría** | **Claude** | **WIP — Cortes 1-2/3 DONE local 2026-07-24 (Claude Opus 4.8).** `CAL-001`. Corte 1 (commit `56bbef2`): `PoliticaLimitesConversacion` (política determinista sin E/S). Corte 2: `ResolvedorTransicionConversacion` (interpretación de la situación + decisión de la siguiente acción, detectores de intención dentro). Todo detrás de `IOrquestadorConversacion`; sin cambiar flujo, flags, mensajes, orden, persistencia ni contratos. Backend verde 462 (410+52; +39), format limpio; Corte 2 sin commit aún. **Próximo = Corte 3 `ProcesadorResultadoEvaluacion` (efectos posteriores).** Spec `P-15_Refactor_Orquestador_Conversacional.md`. |
| **15d** | **`P-16` descomponer página de campañas** | **Remediación auditoría** | **Claude** | **TODO — después de P-15.** `CAL-002`: separar listado, formulario, mensajes, preguntas, participantes y configuración en componentes con entradas/salidas; ruta, API y permisos intactos. Spec `P-16_Refactor_Pagina_Campanias.md`. |
| **15e** | **`P-17` errores API uniformes** | **Remediación auditoría** | **Codex** | **TODO — después de P-16.** `API-001`: todo error directo usa `ErrorRespuesta` y `X-Correlation-Id`, sin revelar detalle sensible. Spec `P-17_Errores_API_Uniformes.md`. |
| **15f** | **`P-18` nombre accesible de controles** | **Remediación auditoría** | **Claude** | **TODO — después de P-17.** `UXA11Y-001`: selección de envíos, tags y CSV con etiqueta o nombre ARIA contextual. Spec `P-18_Controles_Con_Nombre_Accesible.md`. |
| **15g** | **`P-19` anuncios de estados dinámicos** | **Remediación auditoría** | **Codex** | **TODO — después de P-18.** `UXA11Y-002`: patrón común para error/éxito y asociación a campos, sin anuncios duplicados. Spec `P-19_Estados_Dinamicos_Accesibles.md`. |
| **15h** | **`P-20` pestañas accesibles de campañas** | **Remediación auditoría** | **Claude** | **TODO — después de P-19 y P-16.** `UXA11Y-003`: patrón ARIA completo, foco móvil y teclado en el detalle de campaña. Spec `P-20_Pestanas_Accesibles_Campanias.md`. |
...
1. **Continuar `P-15` — Corte 3/3 del refactor del orquestador.** Los Cortes 1 (`PoliticaLimitesConversacion`) y 2 (`ResolvedorTransicionConversacion`) ya están DONE local y verdes (462). Sigue el **Corte 3: `ProcesadorResultadoEvaluacion`** — extraer los efectos posteriores a la evaluación (persistencia de respuesta/evaluación, compilación Markdown, mensajes de repregunta/cierre y logs de seguridad) conservando el orden observable de persistir/enviar/registrar, sin duplicar envíos ni transiciones. Parte de `P-15_Refactor_Orquestador_Conversacional.md`, conserva la fachada `IOrquestadorConversacion` y mantén las 371 pruebas de regresión de mensajes/persistencia verdes. No cambies textos, reglas conversacionales, flags ni contratos. Al cerrar el Corte 3, marca P-15 DONE y rota a `P-16`. `I-12`, `I-13` e `I-14` mantienen sus bloqueos externos: no crear, inferir ni hardcodear etiquetas mientras falte el catálogo GHT.
