...
> **ESTADO VIGENTE 2026-07-25 — pista de auditoría: `P-18` COMPLETO local; siguiente `P-19`:** `P-15`, `P-16`, `P-17` y **`P-18`** permanecen DONE. `P-18` (`UXA11Y-001`) añadió nombres accesibles a la selección total y por fila de envíos, a los campos de etiquetas y al selector CSV con instrucciones vinculadas. No cambian datos, APIs, permisos ni acciones. Se verificó con Node temporal 24.15.0 y Angular local: formato, build y **15 pruebas frontend verdes**. **Próximo ejecutable: `P-19`** (`UXA11Y-002`), seguido por P-20. `I-12` continúa bloqueada por seeds, `I-13` espera decisión GHT e `I-14` sigue bloqueada por el catálogo consolidado de etiquetas: **no crear, inferir ni hardcodear etiquetas**.
...
| **15g** | **`P-19` anuncios de estados dinámicos** | **Remediación auditoría** | **Codex** | **NEXT — después de P-18.** `UXA11Y-002`: patrón común para error/éxito y asociación a campos, sin anuncios duplicados. Spec `P-19_Estados_Dinamicos_Accesibles.md`. |
...
1. **Ejecutar P-19 — estados dinámicos accesibles (`UXA11Y-002`).** P-18 ya está cerrada y P-20 espera después el patrón ARIA de pestañas. En este entorno la vía verificada para Angular es Node temporal 24.15.0 (`npx --yes --package=node@24.15.0 -c "node .\\node_modules\\@angular\\cli\\bin\\ng.js <comando>"`) con los ejecutables locales: build y test son reproducibles. Leer P-19, trazar errores, confirmaciones y validaciones de formulario, introducir un patrón común que anuncie cada cambio una sola vez, cubrir regresión y repetir build/test/format. `I-12`, `I-13` e `I-14` mantienen sus bloqueos externos: no crear, inferir ni hardcodear etiquetas mientras falte el catálogo GHT.
