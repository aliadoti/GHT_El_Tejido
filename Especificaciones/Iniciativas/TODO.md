...
**Vas a implementar la iniciativa objetivo de este TODO: `ID-INICIATIVA=P-13` (umbral de cierre por campaña). AGENTE ASIGNADO: `Claude`. VENTANA: Sprint 1b-2.** No es un desarrollo desde cero: continúas un MVP vivo. **Contexto ya HECHO y verificado:** P-03, P-10, D5 (baseline real pendiente), I-16, I-08 backend **e I-08 UI (2026-07-20, Claude Sonnet 5)**, I-06 diseño+implementación local, I-09 diseño **e I-09 core**, **I-05 parafraseo DONE local (2026-07-20, Codex)**: `configConversacional.parafraseo` default `false` + kill-switch `Conversacion:Parafraseo`, salida/persistencia opcional y truncado por frase; D5 real contra staging pendiente. **I-03 follow-ups eje débil DONE local (2026-07-21, Claude Fable 5):** pista de foco en el `system` (siempre activa) + `CalculadorEjeDebil` (server-side) + `FiltroSalidaRubrica` (siempre-on, capa 2) que reemplaza retro/repregunta con fuga por textos neutros y registra `LogSeguridad(fuga_rubrica)`; sin cambio de contratos `03`/`04`/`08 §4`; D5 real contra staging pendiente (no bloquea el cierre local). I-01 queda BLOCKED para activación real. **Tu objetivo ahora es P-13:** su diseño ya está DONE (2026-07-15); lee su spec y el índice; falta la implementación (override `configConversacional.umbralCierreAnticipado` por campaña, con el global como default/kill-switch); confirma con el usuario cualquier decisión de diseño antes de codificar. Al terminar, rota este TODO al siguiente ítem según §4 y actualiza siempre cabecera, tabla, §8 y AVANCES.md.
...
| 14 | `P-13` umbral de cierre por campaña | Sprint 1b–2 | Claude | **← ACTUAL (objetivo de este TODO)** — **ADELANTADA del post-Hito (decisión del usuario 2026-07-20)**: habilita calibrar I-01 en una campaña de prueba (reversible, sin flip global); diseño DONE 2026-07-15 |
...
1. Identifica la iniciativa objetivo: **`P-13` umbral de cierre por campaña** — agente **Claude**, Sprint 1b-2. Diseño ya DONE (2026-07-15); adelantada por decisión del usuario (2026-07-20) para habilitar calibrar `I-01` en una campaña de prueba sin flip global. Leer `AVANCES.md`, `00_Indice_y_Plan_de_Ejecucion.md` (§2/§3) y `Iniciativas/P-13_*.md`. **`I-03` follow-ups eje débil quedó DONE local 2026-07-21** (pista de foco + filtro de fuga de rúbrica siempre-on; D5 real contra staging pendiente, no bloquea el cierre). Si la spec plantea una decisión de diseño (opción A/B/C), confírmala con el usuario antes de codificar, y rota el handoff al siguiente ítem ejecutable al cerrar.
