...
1. **No iniciar código sin un nuevo insumo externo.** La pista de remediación `P-15` a `P-20` está DONE local. El siguiente trabajo depende de: seeds de Felipe para `I-12`; decisión GHT agnóstica-vs-tailored para `I-13`; o catálogo oficial de etiquetas (nombre, tipo, descripción opcional y estado) para `I-14`. Al recibir uno, actualizar la cabecera, la tabla y este paso antes de implementarlo. No crear, inferir ni hardcodear etiquetas.
