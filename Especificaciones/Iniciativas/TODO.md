...
**Vas a implementar la iniciativa objetivo de este TODO: `ID-INICIATIVA=I-08` (carga masiva de participantes — backend). AGENTE ASIGNADO: `Codex`.** No es un desarrollo desde cero: continúas un MVP vivo. **Contexto (Sprint 1a, ya HECHO y verificado):** `P-03` reinicio de datos (`SUPUESTOS.md#reinicio-datos`), `P-10` completo — cupos (`#guardrails-cupos-conversacion`), rate por número (`#rate-numero-entrante`) y costo LLM por campaña (`#cupo-costo-llm`), **`D5` banco de calibración de prompts** (backend/tooling verde en CI; librería `src/ElTejido.Calibracion` + golden set `tests/Calibracion/golden-set.json` de 24 entradas sin PII + runner opt-in `[Trait("Category","Calibracion")]` fuera de CI; `SUPUESTOS.md#banco-calibracion-d5`; **baseline pendiente de un corrido real contra staging**, ver `tests/Calibracion/README.md`) e **`I-16` fix de calificación en Markdown** (Markdown toma la evaluación más reciente por `fecha DESC`, regresión determinística verde). **Tu objetivo ahora es `I-08`** (spec `Especificaciones/Iniciativas/I-08_Carga_Masiva_Participantes.md`): implementar el backend de carga masiva de participantes desde archivo, empezando por el contrato aditivo `04 §5.1` en commit aparte y luego el servicio/parser con reporte por fila, admin+CSRF y pruebas. **Rotación de agentes: al terminar I-08 backend, deja este TODO listo para el siguiente ítem según §4 (`I-06` diseño → Claude), siguiendo el orden y la alternancia Codex/Claude.**
...
| 5 | **`I-08` carga masiva (backend)** | Sprint 1a | **Codex** | **← ACTUAL (objetivo de este TODO)** |
...
1. Identifica la iniciativa objetivo: **`I-08` (carga masiva de participantes — backend)** — agente **Codex**. `P-03`, `P-10`, **`D5`** e **`I-16`** ya están HECHOS (D5 verificado backend/tooling; baseline pendiente de un corrido real contra staging — ver `AVANCES.md`, `SUPUESTOS.md#banco-calibracion-d5`, tabla §4). Lee la spec `I-08_Carga_Masiva_Participantes.md`; empieza por **actualizar el contrato aditivo `04 §5.1` en commit aparte** para `POST /api/admin/usuarios/carga-masiva`, y luego implementa el backend mínimo: parser server-side, servicio de aplicación, reporte por fila, admin+CSRF y pruebas unitarias/integration. Respeta las dependencias duras (§3). **Al terminar, reescribe la cabecera y §8 de este TODO para apuntar al ítem 6 (`I-06` diseño, agente Claude) y haz el handoff (§5–§6).**
