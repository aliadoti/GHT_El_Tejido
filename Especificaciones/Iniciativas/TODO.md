...
**Vas a implementar la iniciativa objetivo de este TODO: `ID-INICIATIVA=I-06` (multi-idea — implementación). AGENTE ASIGNADO: `Codex`. VENTANA: Sprint 1b.** No es un desarrollo desde cero: continúas un MVP vivo con el **diseño de I-06 ya cerrado** (`SUPUESTOS.md#multi-idea-i06-diseno`, spec `I-06_Multi_Idea_N_Registros.md`). **Contexto (Sprint 1a, ya HECHO y verificado):** `P-03` reinicio de datos (`SUPUESTOS.md#reinicio-datos`), `P-10` completo — cupos (`#guardrails-cupos-conversacion`), rate por número (`#rate-numero-entrante`) y costo LLM por campaña (`#cupo-costo-llm`), **`D5` banco de calibración de prompts** (backend/tooling verde en CI; `SUPUESTOS.md#banco-calibracion-d5`; **baseline pendiente de un corrido real contra staging**), **`I-16` fix de calificación en Markdown** (regresión determinística verde), **`I-08` carga masiva de participantes — backend** (`04 §5.1` + endpoint CSV; verde 335), **`I-06` multi-idea — diseño** (`SUPUESTOS.md#multi-idea-i06-diseno`), **`I-09` tejido colectivo — diseño** (`SUPUESTOS.md#tejido-colectivo-i09-diseno`) e **`I-01` umbral de cierre anticipado — runbook + observabilidad + regresión** (mecanismo ya existía; nuevo `LogSeguridad(cierreUmbralAnticipado)` + `Runbook_I-01_Umbral_Cierre_Anticipado.md`; **cierre real BLOCKED** en baseline D5 real + rúbrica congelada I-11 + flip humano en staging; `SUPUESTOS.md#activacion-umbral-i01`). **Tu objetivo ahora es `I-06` (implementación)**: implementar el puerto `ISegmentadorIdeas`, el fallback 1-idea, los campos aditivos `Respuesta.ideaIndice`/`respuestaPadreId` (`03 §3.8`, ya documentados), ids determinísticos por idea, Markdown por idea y las guardas anti-fragmentación, gateado por `configConversacional.segmentacionIdeas` (por campaña, default off) + kill-switch global `Conversacion:SegmentacionIdeas`. **Dimensiona los cupos P-10:** un turno multi-idea consume `1 + N` llamadas LLM; emite métricas de segmentación separadas de evaluación (`10 §6.2`). Criterio de salida: multi-idea funcional en staging bajo flag, con no-determinismo probado y costo por conversación medido. Si aparece una decisión A/B/C o un cambio de contrato no previsto en el diseño, **confírmalo con el usuario antes de cerrarlo**. **Rotación de agentes: al terminar I-06 implementación, deja este TODO listo para el siguiente ítem según §4 (`I-09` tejido colectivo — core → Claude, Sprint 1b), siguiendo el orden y la alternancia Codex/Claude.**
...
| 9 | **`I-06` multi-idea (implementación)** | Sprint 1b | **Codex** | **← ACTUAL (objetivo de este TODO)** (diseño ya cerrado; implementar `ISegmentadorIdeas` + fallback + Markdown por idea bajo flag) |
...
1. Identifica la iniciativa objetivo: **`I-06` (multi-idea — implementación)** — agente **Codex**, Sprint 1b. Todo el Sprint 1a más **`I-01`** ya están HECHOS (I-01 avanzado al máximo posible: runbook + observabilidad + regresión; su cierre real queda BLOCKED en baseline D5 real + freeze I-11 + flip humano — ver `AVANCES.md`, `SUPUESTOS.md#activacion-umbral-i01`, `Runbook_I-01_Umbral_Cierre_Anticipado.md`, tabla §4). **El diseño de I-06 ya está cerrado** (`SUPUESTOS.md#multi-idea-i06-diseno`, spec `I-06_Multi_Idea_N_Registros.md`): implementa `ISegmentadorIdeas` (corre antes de `IEvaluadorLlm`, `05 §4.3`), el fallback 1-idea, los campos aditivos `Respuesta.ideaIndice`/`respuestaPadreId` (`03 §3.8`), ids determinísticos `resp_<padre>_<idx>`, Markdown por idea (`09`) y las guardas anti-fragmentación (`Conversacion:MaxIdeasPorMensaje`, `LongitudMinimaIdea`), gateado por `configConversacional.segmentacionIdeas` (por campaña, default off) + kill-switch global `Conversacion:SegmentacionIdeas`. **Dimensiona los cupos P-10** (un turno = `1 + N` llamadas LLM) y emite métricas de segmentación separadas de evaluación (`10 §6.2`). Respeta la regla de contratos aditivos (§2) y las dependencias duras (§3). **Al terminar, reescribe la cabecera y §8 de este TODO para apuntar al ítem 10 (`I-09` tejido colectivo — core, Sprint 1b, agente Claude) y haz el handoff (§5–§6).**
