...
> **ESTADO VIGENTE 2026-07-24 — pista de auditoría: backend AGOTADO, quedan solo FRONTEND:** **`P-15` COMPLETO** (3/3 cortes DONE local, orquestador como coordinadora de `PoliticaLimitesConversacion`/`ResolvedorTransicionConversacion`/`ProcesadorResultadoEvaluacion`; commits `56bbef2`/`3e5cc9a`/`4a8e9eb`). **`P-17` DONE local** (`API-001`: `ResultadoError` uniforma el cuerpo de error + correlationId en las 4 rutas con resultados directos; backend verde **469**, format limpio; sin commit aún). Se ejecutó P-17 antes que P-16 por decisión del usuario, dado que P-16/P-18/P-19/P-20 son **frontend Angular** y `ng build`/`ng test` están bloqueados en este entorno. **Próximo:** los ítems restantes de auditoría (**P-16, P-18, P-19, P-20**) son todos frontend y requieren un entorno con Angular operativo (o aceptar validación por CD). `I-12` continúa bloqueada por seeds, `I-13` espera decisión GHT e `I-14` sigue bloqueada por el catálogo consolidado de etiquetas: **no crear, inferir ni hardcodear etiquetas**.
...
| **15c** | **`P-15` refactor del orquestador conversacional** | **Remediación auditoría** | **Claude** | **DONE local 2026-07-24 (Claude Opus 4.8) — 3/3 cortes.** `CAL-001`. Corte 1 (`56bbef2`): `PoliticaLimitesConversacion` (política sin E/S). Corte 2 (`3e5cc9a`): `ResolvedorTransicionConversacion` (transición). Corte 3 (sin commit aún): `ProcesadorResultadoEvaluacion` (efectos posteriores: persistencia/Markdown/telemetría; compuesto `PersistirRespuestaEvaluadaAsync` unifica normal+segmentado). Fachada `IOrquestadorConversacion` intacta; sin cambio de flujo, flags, mensajes, orden, persistencia ni contratos. Backend verde 468 (416+52; +45), format limpio. |
| **15d** | **`P-16` descomponer página de campañas** | **Remediación auditoría** | **Claude** | **DIFERIDA en este entorno (frontend).** `CAL-002`: separar listado, formulario, mensajes, preguntas, participantes y configuración en componentes con entradas/salidas; ruta, API y permisos intactos. Requiere `ng build`/`ng test` (bloqueados aquí por `@esbuild`/WSL/OneDrive); no verificable localmente. Prerequisito de P-20. Spec `P-16_Refactor_Pagina_Campanias.md`. |
| **15e** | **`P-17` errores API uniformes** | **Remediación auditoría** | **Claude** | **DONE local 2026-07-24 (Claude Opus 4.8).** `API-001`: `ResultadoError : IResult` uniforma el cuerpo `ErrorRespuesta` (04 §3) + `correlationId` en `EndpointsAdminEnvios`/jobs, `EndpointsPreparacion`, `FiltroClaveDiagnostico` y `EndpointsWebhook` (403→FORBIDDEN, 401→UNAUTHENTICATED, 404→NOT_FOUND). Sin cambio de códigos HTTP, auth ni respuestas exitosas; sin filtrar secreto/firma/token. Backend verde 469 (+1 int), format limpio; sin commit aún. Spec `P-17_Errores_API_Uniformes.md`. |
...
1. **Decidir el entorno para la parte FRONTEND de la auditoría.** `P-15` (COMPLETO) y `P-17` (DONE local) cerraron todo lo backend de la pista de remediación. Lo que queda —**P-16 `CAL-002`, P-18 `UXA11Y-001`, P-19 `UXA11Y-002`, P-20 `UXA11Y-003`**— es **todo frontend Angular**, y en este entorno `ng build`/`ng test` están bloqueados (mismatch `@esbuild` bajo WSL/OneDrive), por lo que la descomposición y la accesibilidad de plantillas **no se pueden verificar localmente** (solo `prettier` + `tsc --noEmit`, que no chequean bindings de plantilla) y su único build real sería el CD (que despliega a producción). **Opciones:** (a) retomar P-16→P-18→P-19→P-20 en un entorno con Angular operativo (recomendado); (b) si se acepta validar por CD, ejecutarlas aquí panel por panel con `prettier`/`tsc` + revisión manual, empezando por P-16 (P-20 depende de su estructura). No quedan ítems backend de auditoría. `I-12`, `I-13` e `I-14` mantienen sus bloqueos externos: no crear, inferir ni hardcodear etiquetas mientras falte el catálogo GHT.
