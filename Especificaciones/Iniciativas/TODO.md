...
**Vas a implementar la iniciativa objetivo de este TODO: `ID-INICIATIVA=I-09` (tejido colectivo — core). AGENTE ASIGNADO: `Claude`. VENTANA: Sprint 1b.** No es un desarrollo desde cero: continúas un MVP vivo con los diseños de I-06 e I-09 cerrados. **Contexto ya HECHO y verificado:** P-03, P-10, D5 (baseline real pendiente), I-16, I-08 backend, I-06 diseño e I-09 diseño; I-01 queda BLOCKED para su activación real en staging. **I-06 implementación local está DONE:** `ISegmentadorIdeas`, fallback 1-idea, campos aditivos, ids deterministas, Markdown por idea, flags, telemetría y portal; build/test/format verdes. Sus flags siguen apagados hasta D5/UAT/costo en staging. **Tu objetivo ahora es I-09 core:** implementar `IBaseConocimientoCampania` y solo la Opción A léxica aprobada, con recuperación de resúmenes anonimizados, inyección delimitada como dato no confiable, sanitización/presupuesto, degradación autocontenida, flag por campaña `configConversacional.tejidoColectivo` (default off) + kill-switch global `Conversacion:TejidoColectivo`, consentimiento P-07 y observabilidad de costo/latencia. No implementar embeddings (Opción B diferida). Si aparece una decisión A/B/C o un cambio de contrato no previsto en el diseño, **confírmalo con el usuario antes de cerrarlo**. **Al terminar, rota este TODO al siguiente ítem según §4 y actualiza siempre cabecera, tabla, §8 y AVANCES.md.**
...
| 10 | **`I-09` tejido colectivo (core)** | Sprint 1b | **Claude** | **← ACTUAL (objetivo de este TODO)** (Opción A léxica, flags apagados, consentimiento y observabilidad) |
...
1. Identifica la iniciativa objetivo: **`I-09` (tejido colectivo — core)** — agente **Claude**, Sprint 1b. I-06 está implementada localmente y permanece apagada hasta D5/UAT/costo en staging; I-01 continúa BLOCKED para el flip humano. Lee `AVANCES.md`, `I-09_Tejido_Colectivo.md` y `SUPUESTOS.md#tejido-colectivo-i09-diseno`. Implementa exclusivamente la **Opción A léxica** acordada: `IBaseConocimientoCampania.RecuperarAsync`, recuperación de resúmenes anonimizados derivados de datos existentes, inyección delimitada/sanitizada como dato no confiable, presupuesto de tokens, fallback autocontenido, `configConversacional.tejidoColectivo=false` por defecto + kill-switch `Conversacion:TejidoColectivo`, consentimiento P-07 y observabilidad de aportes/costo/latencia. No implementes embeddings (Opción B diferida). Respeta contratos aditivos y confirma cualquier nueva decisión A/B/C. **Al terminar, actualiza siempre cabecera, fila §4, este §8 y `AVANCES.md` para el siguiente ítem ejecutable.**
