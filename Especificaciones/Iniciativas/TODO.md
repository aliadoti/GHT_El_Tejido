...
> **ESTADO VIGENTE 2026-07-24 — `ID-INICIATIVA=P-15`, Corte 2/3 — PRÓXIMO EJECUTABLE:** por decisión del usuario se abre una pista inmediata de remediación de auditoría. `P-15` corrige la cohesión interna del orquestador sin alterar su contrato ni el flujo conversacional, en 3 cortes. **Corte 1/3 DONE local 2026-07-24 (Claude Opus 4.8):** política determinista sin E/S extraída a `PoliticaLimitesConversacion` (umbral/madurez/corte/elegibilidad de mejora); orquestador delega sin cambiar comportamiento; backend verde **443** (391 unit + 52 integration; +20), format limpio; sin commit/push aún. **Próximo = Corte 2/3 `ResolvedorTransicionConversacion`** (resolución de la transición) y luego Corte 3 `ProcesadorResultadoEvaluacion` (efectos posteriores). Al cerrar los 3, rotar a `P-16`. `I-12` continúa bloqueada por seeds, `I-13` espera decisión GHT e `I-14` sigue bloqueada por el catálogo consolidado de etiquetas: **no crear, inferir ni hardcodear etiquetas**.
...
| **15c** | **`P-15` refactor del orquestador conversacional** | **Remediación auditoría** | **Claude** | **WIP — Corte 1/3 DONE local 2026-07-24 (Claude Opus 4.8).** `CAL-001`: `PoliticaLimitesConversacion` (política determinista sin E/S: umbral/madurez/corte/elegibilidad de mejora) extraída detrás de `IOrquestadorConversacion`; sin cambiar flujo, flags, mensajes, orden, persistencia ni contratos. Backend verde 443 (391+52; +20), format limpio; sin commit aún. **Próximo = Corte 2 `ResolvedorTransicionConversacion`, luego Corte 3 `ProcesadorResultadoEvaluacion`.** Spec `P-15_Refactor_Orquestador_Conversacional.md`. |
...
1. **Continuar `P-15` — Corte 2/3 del refactor del orquestador.** El Corte 1 (`PoliticaLimitesConversacion`, política determinista sin E/S) ya está DONE local y verde (443). Sigue el **Corte 2: `ResolvedorTransicionConversacion`** (resolución de la transición conversacional: interpretación de la situación + decisión de la siguiente acción) y luego el **Corte 3: `ProcesadorResultadoEvaluacion`** (efectos posteriores a la evaluación, conservando el orden de persistir/enviar/registrar). Parte de `P-15_Refactor_Orquestador_Conversacional.md`, conserva la fachada `IOrquestadorConversacion` y mantén las 371 pruebas de regresión de mensajes/persistencia verdes en cada corte. No cambies textos, reglas conversacionales, flags ni contratos. Al cerrar los 3 cortes, rota a `P-16`. `I-12`, `I-13` e `I-14` mantienen sus bloqueos externos: no crear, inferir ni hardcodear etiquetas mientras falte el catálogo GHT.
