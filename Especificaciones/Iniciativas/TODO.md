...
**Vas a implementar la iniciativa objetivo de este TODO: `ID-INICIATIVA=I-01` (activar umbral de cierre en staging). AGENTE ASIGNADO: `Claude`.** No es un desarrollo desde cero: continúas un MVP vivo. **Contexto (Sprint 1a, ya HECHO y verificado):** `P-03` reinicio de datos (`SUPUESTOS.md#reinicio-datos`), `P-10` completo — cupos (`#guardrails-cupos-conversacion`), rate por número (`#rate-numero-entrante`) y costo LLM por campaña (`#cupo-costo-llm`), **`D5` banco de calibración de prompts** (backend/tooling verde en CI; librería `src/ElTejido.Calibracion` + golden set `tests/Calibracion/golden-set.json` de 24 entradas sin PII + runner opt-in `[Trait("Category","Calibracion")]` fuera de CI; `SUPUESTOS.md#banco-calibracion-d5`; **baseline pendiente de un corrido real contra staging**, ver `tests/Calibracion/README.md`), **`I-16` fix de calificación en Markdown** (regresión determinística verde), **`I-08` carga masiva de participantes — backend** (contrato aditivo `04 §5.1` + endpoint CSV; `SUPUESTOS.md#carga-masiva-participantes`; verde 335), **`I-06` multi-idea — diseño** (`SUPUESTOS.md#multi-idea-i06-diseno`) e **`I-09` tejido colectivo — diseño** (puerto `IBaseConocimientoCampania`, flag per-campaña `configConversacional.tejidoColectivo` en `03 §3.3` aditivo, kill-switch global `Conversacion:TejidoColectivo`, recuperación Opción A léxica con B embeddings diferida, inyección como dato delimitado `08 §3.2/§5.9`, degradación limpia; `SUPUESTOS.md#tejido-colectivo-i09-diseno`). **Tu objetivo ahora es `I-01` (activar umbral de cierre en staging)**: I-01 **ya existe en código** (`Conversacion:UmbralCierreAnticipado`, default off — ver `Iniciativas/00_Indice §1.2`); el trabajo es **activación + calibración en staging**, no desarrollo de un umbral nuevo. **Depende de `P-10 cupos` ✓ y de la calibración D5** (baseline pendiente de un corrido real). **Regla D2: no se retira el tope determinístico de revisiones hasta que los cupos estén activos en producción.** Si la activación plantea decisiones A/B/C o toca contratos, **confírmalas con el usuario antes de cerrarlas**. **Rotación de agentes: al terminar I-01, deja este TODO listo para el siguiente ítem según §4 (`I-06` multi-idea — implementación → Codex, Sprint 1b), siguiendo el orden y la alternancia Codex/Claude.**
...
| 8 | **`I-01` activar umbral en staging** | Sprint 1a | **Claude** | **← ACTUAL (objetivo de este TODO)** (depende de `P-10 cupos` ✓ y calibración D5; I-01 ya existe en código, es activación + calibración) |
...
1. Identifica la iniciativa objetivo: **`I-01` (activar umbral de cierre en staging)** — agente **Claude**. `P-03`, `P-10`, **`D5`**, **`I-16`**, **`I-08` backend**, **`I-06` diseño** e **`I-09` diseño** ya están HECHOS (D5 verificado backend/tooling; baseline pendiente de un corrido real contra staging — ver `AVANCES.md`, `SUPUESTOS.md#banco-calibracion-d5`/`#carga-masiva-participantes`/`#multi-idea-i06-diseno`/`#tejido-colectivo-i09-diseno`, tabla §4). **I-01 NO es código nuevo:** ya existe (`Conversacion:UmbralCierreAnticipado`, default off; ver `Iniciativas/00_Indice §1.2` y `SUPUESTOS.md#orquestador-conversacional`). El trabajo es **activar y calibrar el umbral en staging** con el banco D5 como árbitro, respetando la **regla D2** (no retirar el tope determinístico de revisiones hasta que los cupos P-10 estén activos en producción) y las dependencias duras (§3: `P-10 cupos` ✓ → `I-01`). Documenta la activación/calibración donde corresponda y **confirma con el usuario cualquier decisión A/B/C o cambio de contrato antes de cerrarla**. **Al terminar, reescribe la cabecera y §8 de este TODO para apuntar al ítem 9 (`I-06` multi-idea — implementación, Sprint 1b, agente Codex) y haz el handoff (§5–§6).**
