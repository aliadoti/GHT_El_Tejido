...
> **ESTADO VIGENTE 2026-07-25 — pista de auditoría `P-15` a `P-20` COMPLETA local:** `P-20` (`UXA11Y-003`) completa el patrón ARIA del detalle de campañas: pestañas y panel relacionado, foco móvil y teclado con Flecha, Inicio y Fin; la página conserva una única fuente de verdad para la sección activa. No cambian rutas, APIs, DTO, permisos ni acciones. Se verificó con Node temporal 24.15.0 y Angular local: formato, build y **19 pruebas frontend verdes**. No hay una tarea de código ejecutable en esta pista: `I-12` continúa bloqueada por seeds, `I-13` espera decisión GHT e `I-14` sigue bloqueada por el catálogo consolidado de etiquetas: **no crear, inferir ni hardcodear etiquetas**.
...
| 24 | **`P-21` multi-número de WhatsApp** | A coordinar (fuera de ruta crítica) | por asignar | **TODO — spec lista 2026-07-25, implementación pendiente.** Misma WABA/App, dos números indistintos (QA/producción). Capturar `metadata.phone_number_id` → responder por el número entrante; `IWhatsAppGateway` con emisor opcional (default predeterminado); número saliente **por campaña** (`configConversacional.numeroWhatsAppSaliente`, aditivo `03 §3.3`). Sin secretos nuevos; cero cambio por defecto. Spec `P-21_Multi_Numero_WhatsApp.md`, `SUPUESTOS.md#multi-numero-whatsapp-p21`. **Ejecutable ya** (no depende de insumos externos); confirmar ventana con el usuario. |
...
1. **Ítem ejecutable disponible sin insumos externos: `P-21` multi-número de WhatsApp (§4 fila 24).** Spec lista `Iniciativas/P-21_Multi_Numero_WhatsApp.md` (implementación pendiente); decisiones de diseño ya confirmadas con el usuario (`SUPUESTOS.md#multi-numero-whatsapp-p21`). Confirmar la ventana con el usuario y seguir el bucle de §3 en los pasos pequeños de la spec §8. Los demás ítems ordinarios siguen bloqueados por terceros: seeds de Felipe para `I-12`; decisión GHT agnóstica-vs-tailored para `I-13`; catálogo oficial de etiquetas para `I-14` (no crear, inferir ni hardcodear etiquetas). La pista de remediación `P-15` a `P-20` está DONE local.
