...
> **🟡 `DT-P33-01` — BASE DESPLEGADA; HOTFIX DETERMINISTA LISTO LOCAL (2026-08-20).** El commit
> `ff54bb0` y los gates semántico/visibilidad están activos en el ambiente. Un caso real demostró que
...
> semántica. Siete alias ingleses quedaron en semilla y en el catálogo inglés v3 ya importado como
> **borrador**; español v3 se conservó sin cambios. Validación: build Release `-warnaserror`, **1053
> unitarias + 121 integración**, formato y diff verdes. **Siguiente:** commit único → deploy → activar
> inglés v3 → smoke abierto/cerrado/mixto → QAS/25, D5 `n=3`, costo/latencia y acta.
>
...
| **DT-P33-01** | **Clasificación semántica de consulta de idea** | **BASE DESPLEGADA; HOTFIX LOCAL PENDIENTE DE PUBLICAR** | **Codex** | `ff54bb0` desplegado con gates semántico/visibilidad ON. Hotfix: afinidad P-33 + alias exacto prevalecen sin LLM; mixto sigue como aporte. Inglés v3 importado en borrador, no activo. Gate local: build, 1053 unitarias + 121 integración, formato/diff verdes. Siguiente: commit/deploy, activar v3, smoke y luego QAS/25+D5+acta. |
