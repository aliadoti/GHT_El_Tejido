...
> devuelve participación, embudo, distribución de calificaciones, cobertura por pregunta y temas con
> **los mismos filtros del listado**, y pasa por el mismo `ConsultaResultadosCompartida`: `totalIdeas`
> coincide siempre con el `total` de la tabla (§8.9). `convocados` es la convocatoria completa —el
> denominador de «¿cuánta gente participó?»— y `conIdeas` sale del filtro; los tramos cubren la escala
> del snapshot de rúbrica —el último incluye su extremo— y sin snapshot usan los valores observados;
> `temas` desempata alfabéticamente para ser estable. **La marca del umbral solo se dibuja cuando es
> uniforme**: una línea que no vale para parte del histograma es peor que ninguna. Portal: panel
> **plegado** con las cinco cifras en el encabezado y una **tabla con barra** por gráfico, sin
> librerías nuevas y con el dato legible sin ver el color (P-18/P-19). Backend **1095 unitarias + 145
> de integración**; portal **89 pruebas en 11 archivos**. Decisiones en
> `SUPUESTOS.md#resumen-campania-p-34` y `#cierre-p-34`.
>
> **🟢 `P-34` CORTE 5/6 EXPORTACIÓN — DONE LOCAL 2026-08-21 (Claude Opus 5).** Contrato `04 §5.8` en
> commit previo `c9181a6`. Entran `GET /admin/campanias/{id}/exportar` —ideas, aportes o evaluaciones,
> en `xlsx` o `csv`, con `anonimizado` y **los mismos filtros de la pantalla**— y `documentos.zip` con
> un `.md` por idea nombrado `U-000042_Ana-Perez_idea-1.md`. Se ignoran `page`/`pageSize`: exportar
> media página sería una trampa. Para que el archivo y la vista no diverjan, ambos pasan por el nuevo
> `ConsultaResultadosCompartida` y **el listado se refactorizó** para usarlo. El archivo **declara su
> alcance** —campaña, filtros, orden, total, fecha y quién exportó— como primera hoja o como líneas
> `#` del CSV; `csv` en UTF-8 con BOM y nombres de archivo en ASCII. **Tope de 10.000 filas**
> verificado dos veces, con `400` y el total encontrado. `xlsx` y ZIP se arman en archivo temporal
> —sus librerías solo escriben síncrono, y hacerlo sobre el socket está prohibido y bloquea un hilo—
> y se envían asíncronos con `DeleteOnClose`: nunca el archivo entero en memoria. **Anonimizado (D1)**
> sustituye el nombre por el código también en el ZIP. Sin dependencias nuevas (ClosedXML ya estaba
> por I-08). Backend **1088 unitarias + 141 de integración**; portal **87 pruebas en 11 archivos**.
> Decisión en `SUPUESTOS.md#exportacion-resultados-p-34`.
>
...
> embebe `participante` —código legible, nombre, área, empresa, sede, estado y `resuelto`, sin número,
> email ni tags— y `calificacionTotal`/`evaluadaEn`, y acepta `q`, `area`, `empresa`, `sede`, `desde`,
> `hasta`, `calificacionMin/Max`, `confirmada` y `orden`/`dir`. Con eso **desaparece el join del
> maestro de usuarios en el navegador** —origen de H-01/H-02— y filtrar por área deja de mentir sobre
> el `total`. `ConsultaIdeasResultados` (Application, puro) valida y devuelve **todos** los motivos con
> `400`: un rango mal escrito no es «no hay resultados». Las filas sin dato quedan al final en ambas
> direcciones y todo orden desempata por el orden natural, para que paginar sea estable. El puerto
> suma `ListarUsuariosPorIdsAsync` y `ListarEvaluacionesPorIdsAsync`, con degradación por defecto, y
> el texto o la calificación solo se leen antes de paginar si algún criterio los exige. Portal: barra
> de dos niveles, chips removibles con «Limpiar todo», **filtro en la URL** (H-09) y vacío que nombra
> el filtro; el maestro descargado queda solo como respaldo para un servidor anterior (`§9`). Medición
> con 1.000 ideas y filtros: 1 consulta de ideas + 1 de identidad + 2 de versiones y **0 lecturas
> puntuales**, 342 ms. Backend **1073 unitarias + 131 de integración**; portal **79 pruebas en 10
> archivos**; build, format, diff, `ng build` producción y Prettier verdes. Fuera de alcance a
> propósito: «con/sin documento», «con/sin evaluación» y la UI de ordenamiento (tabla del corte 4).
> Decisión en `SUPUESTOS.md#identidad-y-filtros-servidor-p-34`.
>
> **🟢 `P-34` CORTE 2/6 ESCALA A 1.000 IDEAS — DONE LOCAL 2026-08-20 (Claude Opus 5).** El listado
> resolvía la versión vigente **idea por idea y antes de paginar**; ahora filtra, ordena, **recorta la
> página** y solo entonces resuelve, pidiendo las versiones de esa página en **una sola consulta por
> ids** dentro de la partición (`ListarVersionesDeCampaniaAsync`). El detalle pide los aportes por
> `ideaId` (`ListarRespuestasPorIdeaAsync`) en vez de leer la partición completa de respuestas. Ambos
> métodos del puerto traen **implementación por defecto que degrada al comportamiento anterior**, así
> que un adaptador o doble sin la consulta nativa devuelve lo mismo, solo que más caro. Medición con
> 1.000 ideas y 5.000 aportes sembrados: listado de **1.000 lecturas puntuales a 0** (403 → 332 ms) y
> detalle de **5.000 documentos de respuesta leídos a 5** (701 → 496 ms); es una medición de
> **operaciones**, in-process — las **RU y latencia reales contra Cosmos siguen siendo puerta
> operativa**. Con el listado ya barato, el portal recorre también las páginas de ideas y el desglose
> por estado de H-04 quedó exacto. Backend **1056 unitarias + 125 de integración**; portal **77
> pruebas en 10 archivos**; build Release `-warnaserror`, `dotnet format`, `git diff --check`,
> `ng build` producción y Prettier verdes. Sin contratos, datos ni configuración remota. Decisión en
> `SUPUESTOS.md#escala-listado-ideas-p-34`.
>
...
> siendo no determinista y la frase no estaba catalogada. El hotfix hace prevalecer la afinidad P-33 +
...
> `planes/DT-I20-02_Prompt_Candidato_Evaluacion.md`: texto listo para copiar, tabla de lo que el
...
> humana previa. **No se creó ni activó nada en Cosmos.** Las pruebas de §7.2 corren sobre el
> **evaluador real** con un `ILlmClient` falso dentro del recorrido webhook → cola → orquestador →
> gateway: el caso reportado llega sin `###`, sin `Lo que ya queda claro` / `Lo que todavía falta` /
> `Siguiente ajuste recomendado`, sin `ready_to_save` y con una sola pregunta; el aporte conserva
> `caja #3` carácter por carácter; y la corrida equivalente con campos visibles válidos produce
> **idéntica decisión de negocio**. Backend **925 unitarias + 112 de integración**; regresiones por
> iniciativa verdes (I-18 8, I-19 22+1, I-20 20+3, DT-I20-01 6, P-27 18+2, P-32 16, P-33 4,
> DT-I20-02 9+2). Cierre documental en `05 §4.4.2`, `08 §3.4` y
> `Reglas_Conversacion_y_Participacion.md §2.14`. **Pendiente explícito por costo, credenciales y
> autorización humana:** `QAS/21` y D5 reales.
>
...
> `activa|borrador` con sus problemas, pero cada
...
> Revalidación Azure PASS, evidencia Meta aceptada y cierre operativo OFF confirmado.
> **Siguiente:** `DT-I20-02` corte 1/3. `DT-P32-04` sigue post-green y no bloquea.
...
0. **NO HAY TRABAJO DE CÓDIGO ASIGNADO.** `P-34` quedó completa 6/6 el 2026-08-21. Antes de escribir
   una línea, **acuerda el objetivo con el usuario** entre los candidatos del encabezado (backlog
   `DT-P33-01`, puertas operativas de la convención, o la deuda anotada en
   `SUPUESTOS.md#cierre-p-34`). Si el usuario pide publicar, recuerda que **el push a `main`
   despliega**: van varios commits locales de P-34 sin publicar.
