...
**Vas a implementar la iniciativa objetivo de este TODO: `ID-INICIATIVA=I-16` (fix de calificación en el artefacto Markdown). AGENTE ASIGNADO: `Claude`.** No es un desarrollo desde cero: continúas un MVP vivo. **Contexto (Sprint 1a, ya HECHO y verificado):** `P-03` reinicio de datos (`SUPUESTOS.md#reinicio-datos`), `P-10` completo — cupos (`#guardrails-cupos-conversacion`), rate por número (`#rate-numero-entrante`) y costo LLM por campaña (`#cupo-costo-llm`), y **`D5` banco de calibración de prompts** (backend/tooling verde en CI; librería `src/ElTejido.Calibracion` + golden set `tests/Calibracion/golden-set.json` de 24 entradas sin PII + runner opt-in `[Trait("Category","Calibracion")]` fuera de CI; `SUPUESTOS.md#banco-calibracion-d5`; **baseline pendiente de un corrido real contra staging**, ver `tests/Calibracion/README.md`). **Tu objetivo ahora es `I-16`** (spec `Especificaciones/Iniciativas/I-16_Fix_Calificacion_Markdown.md`): reproducir/verificar y corregir el defecto de la calificación tal como se renderiza en el artefacto Markdown (`09`), con **prueba de regresión determinista** que fije el comportamiento correcto; visible en demo. Cambios solo aditivos y en commit aparte si tocan una spec de contrato (`03`/`04`/`09`). **Rotación de agentes: al terminar I-16, deja este TODO listo para el siguiente ítem según §4 (I-08 backend → Codex), siguiendo el orden y la alternancia Codex/Claude.**
...
| 4 | **`I-16` fix de calificación en Markdown** | Sprint 1a | **Claude** | **← ACTUAL (objetivo de este TODO)** |
...
1. Identifica la iniciativa objetivo: **`I-16` (fix de calificación en el artefacto Markdown)** — agente **Claude**. `P-03`, `P-10` completo y **`D5`** ya están HECHOS (D5 verificado backend/tooling; baseline pendiente de un corrido real contra staging — ver `AVANCES.md`, `SUPUESTOS.md#banco-calibracion-d5`, tabla §4). Lee la spec `I-16_Fix_Calificacion_Markdown.md`; empieza por **reproducir el defecto con una prueba que falle** (rojo) sobre el compilador de Markdown (`09`), luego corrige mínimo y deja la prueba en verde (regresión determinista). Respeta las dependencias duras (§3). **Al terminar, reescribe la cabecera y §8 de este TODO para apuntar al ítem 5 (`I-08` backend, agente Codex) y haz el handoff (§5–§6).**
