...
| 2 | **2a DONE local 2026-08-10:** `Conversacion.Idioma` queda fijado al crear o abrir un ciclo, persiste en Cosmos, conserva `es` en documentos históricos y se expone en Resultados. `ResolutorTextosConversacion` proyecta proveedor + snapshot sin conectarse a salidas visibles. **2b pendiente:** sustituir textos/variantes/frases globales por claves del proveedor y detectar `es/en`; después propagar snapshot a enrutamiento/evaluación. | 2a: round-trip Cosmos, hilo nuevo `en`, transiciones inmutables y regresión legacy con gate OFF. 2b: cada clave se usa; sin hardcodes visibles fuera del respaldo; comandos de salida; fallback correcto y telemetría sin contenido. |
