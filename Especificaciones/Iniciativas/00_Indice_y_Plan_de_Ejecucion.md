...
> **Última revisión: 2026-08-20 — `P-34` ES LA SIGUIENTE IMPLEMENTACIÓN (0/6).** Iniciar por el corte
> 1: H-01/H-02/H-03/H-04 de Resultados, con regresiones y sin adelantar los cortes 2–6. El hotfix
> determinista DT-P33-01 está desplegado: `85b78f8`, tag
> `v1.0.3-convencion`, fue publicado con workflow verde y `/health/ready=ok`; clasificación semántica y
> visibilidad de idea están ON. El caso real de conformidad inglesa
> posterior a consulta abrió un hotfix acotado: afinidad P-33 + alias exacto se resuelven antes del LLM.
> La semilla y el catálogo inglés se ampliaron; la v3 está activa. Gate local: 1053 unitarias + 121
> integración, build/formato/diff verdes. El robustecimiento semántico integral, QAS/25
> abierto/cerrado/mixto, D5 y acta quedan en backlog para una iteración posterior y no bloquean P-34.
>
...
| **DT-P33-01** | [DT-P33-01_Clasificacion_Semantica_Consulta_Idea.md](DT-P33-01_Clasificacion_Semantica_Consulta_Idea.md) | **HOTFIX DESPLEGADO; ROBUSTECIMIENTO INTEGRAL EN BACKLOG — 2026-08-20** | `85b78f8` / `v1.0.3-convencion`, workflow/readiness verdes, gates ON e inglés v3 activo. Pendiente posterior: banco semántico, QAS/25 abierto/cerrado/mixto, D5, costo/latencia y acta. No bloquea P-34. |
| **P-34** | [P-34_Resultados_Filtros_Tabla_y_Exportacion.md](P-34_Resultados_Filtros_Tabla_y_Exportacion.md) | **COMPLETA — 6/6 DONE local (2026-08-20 y 2026-08-21)** | Corte 1 (solo portal, sin contratos): páginas recorridas hasta el `total` del servidor en usuarios/Markdown/respuestas/conversaciones, contadores tomados de `total` con aviso de desglose parcial, y fallo del maestro de participantes visible/reintentable con «Participante no identificado · código». Corte 2 (backend): paginar antes de resolver versiones, versiones de la página en una sola consulta por ids, aportes por `ideaId` y listado de ideas recorrido completo desde el portal; medido con 1.000 ideas, de 1.000 lecturas puntuales a 0 y de 5.000 documentos de respuesta a 5 (RU reales contra Cosmos, puerta operativa pendiente). Backend 1056 + 125; portal 77 pruebas. Corte 3: `participante` embebido y calificación vigente, filtros de servidor (`q`, área, empresa, sede, fechas, calificación, confirmada), `orden`/`dir` con `400` ante consulta inválida, y portal con barra de dos niveles, chips, filtro en la URL y vacío que nombra el filtro; contrato `04 §5.8` en commit previo `98b8bca`. Corte 4 (solo portal): tabla ordenable con orden de servidor y `aria-sort`, agrupación, columnas/densidad/página configurables, paginación honesta, vista lectura conservada, ficha completa (H-05) y línea de tiempo única. Corte 5: `/exportar` (ideas/aportes/evaluaciones en xlsx/csv, anonimizado, filtros de la pantalla) y `documentos.zip`, con el alcance declarado dentro del archivo y tope de 10.000 filas. Corte 6: `/resumen` con los mismos filtros y el mismo alcance que la tabla, panel plegado con las cinco cifras visibles y tabla equivalente por gráfico. **Iniciativa completa**; lo que quedó fuera y la puerta operativa (RU/latencia reales) están en `SUPUESTOS.md#cierre-p-34`. Prioridad alta previa a convención. |
| **P-35** | [P-35_Nombre_de_Saludo_Participantes.md](P-35_Nombre_de_Saludo_Participantes.md) | **COMPLETA LOCAL — 2026-08-21** | Mantiene `nombre` completo y agrega `nombreSaludo` persistido, calculado y editable para `{{nombre}}`. Compatibilidad histórica, portal y backfill idempotente completos; falta ejecutar el backfill remoto con autorización. La columna CSV/XLSX queda como deuda explícita. |
