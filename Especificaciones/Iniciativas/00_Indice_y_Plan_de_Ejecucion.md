...
> **Última revisión: 2026-08-13 — `P-33` ESPECIFICADA Y APROBADA (0/3), implementación inmediata.**
> La consulta «¿cómo va mi idea?» muestra por defecto la activa o, si no existe, la última trabajada;
> no abre menú ni crea un aporte. Los cierres normales muestran la versión vigente. Consultar una idea
> cerrada crea afinidad temporal y una corrección posterior reabre esa misma idea. Gate global OFF,
> opt-outs por campaña, versión exacta I-19, catálogo `es/en` y QAS definidos. Spec: `P-33_*`.
>
...
| **DT-QA-02** | [DT-QA-02_Listado_Evaluaciones_Y_Huerfanas.md](DT-QA-02_Listado_Evaluaciones_Y_Huerfanas.md) | **DONE local 2026-08-08** | `GET /api/admin/evaluaciones` con puerto obligatorio, adaptadores Cosmos/memoria `fecha DESC`, filtros/paginación/resumen y diagnóstico derivado `enlazada`/`huerfana`/`superada`/`sin_version_idea`. El DTO no expone texto libre; `superada` no se cuenta como huérfana (I-16). Build y 814 pruebas no-Calibracion verdes; sin flags, despliegue ni configuración remota. No repara documentos ni agrega UI. **Siguiente prioridad actual: P-33 corte 1.** |
...
`I-19/I-20/P-26/P-29/P-30/P-31/P-32` → `P-33` **ESPECIFICADA 0/3** (consulta y cierre
visible; siguiente iniciativa de código). ·
