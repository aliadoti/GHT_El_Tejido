...
> Última revisión: 2026-07-25 — **nueva iniciativa `P-21` (multi-número de WhatsApp): spec creada, implementación pendiente.** El sistema podrá enviar/responder por dos números bajo la misma WABA/App (uno para pruebas/calidad, otro para producción); número saliente **por campaña** con default global y respuesta **siempre por el número entrante**. Revisión anterior 2026-07-24 — **remediación de auditoría P-15 a P-20 incorporada; P-15 es el próximo ejecutable.** Revisión anterior 2026-07-21 (tarde) — **RE-PRIORIZACIÓN reunión GHT 20-jul** (grabación Fathom
...
| P-21 | [P-21_Multi_Numero_WhatsApp.md](P-21_Multi_Numero_WhatsApp.md) | A coordinar (fuera de ruta crítica) | **TODO — spec lista 2026-07-25, implementación pendiente.** Multi-número de WhatsApp (misma WABA/App): captura `metadata.phone_number_id` para **responder por el número entrante**, `IWhatsAppGateway` con número emisor opcional (default = predeterminado) y número saliente **por campaña** (`configConversacional.numeroWhatsAppSaliente`, aditivo `03 §3.3`). Sin secretos nuevos; cero cambio por defecto. |
