...
| **Cierre conversacional por tiempo (P-29)** | Kill-switch `Conversacion:CierrePorTiempoHabilitado=false`; umbral existente `MinutosInactividadSesion` (campaña ?? global) | Gobierna **solo el aviso de pausa**: apagado, el cierre por inactividad de I-17 opera igual. Un único mensaje por hilo cerrado (el hilo ya está cerrado cuando se envía, así que el barrido no repite). Fuera de la ventana de 24 h o con campaña cerrada administrativamente se omite el envío; nunca se fuerza HSM ni se evalúa una versión sin confirmar. |
