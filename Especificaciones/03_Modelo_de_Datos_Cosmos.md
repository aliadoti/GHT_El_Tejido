...
```json
{
  ...
  "configMarkdown": { "tipoArtefacto": "respuesta" },
  "configConversacional": { "maxRepreguntas": 1, "mensajeCierre": "Gracias. Tu aporte quedó registrado correctamente.", "segmentacionIdeas": false, "tejidoColectivo": false, "parafraseo": false },
...
- `configConversacional.umbralCierreAnticipado` (P-13, **aditivo**, default **ausente/null**): **override por campaña** del umbral de cierre anticipado por calificación alta (`05 §4.4`; fracción de la escala de la rúbrica en `[0,1]`, `<= 0` lo desactiva para esa campaña). Ausente/null = la campaña **hereda** el default numérico global `Conversacion:UmbralCierreAnticipado` (comportamiento actual). El kill-switch operativo independiente `Conversacion:CierreAnticipadoHabilitado` (default `true`) prevalece sobre ambos: `false` apaga el cierre anticipado para todas las campañas. La resolución efectiva es `!habilitado ? off : campaña ?? global`. Documento viejo sin el campo = usa el global. Ver `Iniciativas/P-13_Umbral_Cierre_Por_Campania.md`.
