...
- Decision:
  ...
  - **Mensaje enviado:** saludo configurable (`Conversacion:Mensajes:SaludoPrimerContacto`) + `pregunta.Texto` combinados, como **texto libre** dentro de la ventana (05 §2.2), registrado como `EnvioMensaje` tipo `Inicial` (es, en efecto, la entrega de la pregunta inicial). Se guarda tambien el `Mensaje(in)` del saludo para el hilo, pero NO se llama al LLM ni se persiste `Respuesta`/`Evaluacion`.
