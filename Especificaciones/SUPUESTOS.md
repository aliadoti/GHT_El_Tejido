...
### primer-contacto-pregunta - Cold-start del participante -> responder con la pregunta vigente
- Fecha: 2026-06-16 - Agente/Rol: Claude Code - Arquitecto/Backend - Commit: pendiente
- Contexto: `05 §4` asume flujo business-initiated (la campania envia la pregunta y el participante responde), por lo que el orquestador evaluaba TODO entrante como respuesta. Si el participante inicia en frio (p. ej. "Hola") sin haber recibido la pregunta, evaluar el saludo es mala UX. Decision del usuario 2026-06-16: el primer contacto en frio debe recibir la pregunta vigente; recien el siguiente mensaje se evalua. Esto ademas habilita la prueba real con WhatsApp sin billing (las respuestas dentro de la ventana de 24h son texto libre = gratis; `131042` solo bloquea mensajes que ABREN conversacion).
- Decision:
  - **Discriminador de cold-start:** en `OrquestadorConversacion.ProcesarMensajeEntranteAsync`, se trata como primer contacto en frio cuando `conversacion is null` **y** `participante.EstadoEnvio != EstadoEnvio.Enviado` (el envio inicial de campania nunca se hizo). Si `EstadoEnvio == Enviado` (el participante ya vio la pregunta) se mantiene el comportamiento actual: evaluar.
  - **Como se evita el bucle:** el cold-start **crea y persiste** la `Conversacion` en `EsperandoRespuestaInicial` (no avanza a `Evaluando`). El SIGUIENTE entrante ya halla esa conversacion (`conversacion is not null`) y cae al flujo normal de evaluacion, aunque `EstadoEnvio` siga sin ser `Enviado`. Es decir, el discriminador real del "segundo turno" es la existencia de la conversacion, no el estado de envio.
...
- Alternativa(s) descartada(s): usar `EstadoEnvio` como unico discriminador del segundo turno (provoca bucle: el cold-start no cambia `EstadoEnvio`, asi que el 2.º mensaje volveria a tratarse como cold-start); marcar `EstadoEnvio=Enviado` tras el cold-start (confunde el estado del envio masivo de campania con el contacto entrante); evaluar el saludo y responder con la pregunta a la vez (gasta un turno de evaluacion sobre "Hola", mala señal para la rubrica).
