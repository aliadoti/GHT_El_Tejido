...
- Actualizacion anterior: 2026-08-21 (Claude Opus 5, Backend/Frontend/UX-A11Y/SDET): **`P-34` COMPLETA
  ...
  `totalIdeas` coincide siempre con el `total` del listado (§8.9)—. El panel va **plegado** con las
  cinco cifras visibles, cada gráfico es una **tabla con barra** (sin librerías nuevas, el color nunca
  va solo) y la marca del umbral **solo se dibuja si aplica a todas las barras**. Backend **1095
  unitarias + 145 de integración**; portal **89 pruebas en 11 archivos**; build, format, diff,
  `ng build` producción y Prettier verdes. **P-34 queda completa (6/6) y sin trabajo de código
  pendiente**; lo que sigue lo define el usuario. Puerta operativa abierta: medición de RU/latencia
  reales contra Cosmos.
- Actualizacion anterior: 2026-08-21 (Claude Opus 5, Arquitecto/Backend/Frontend/SDET): **`P-34` CORTE
  5/6 — DONE LOCAL.** **Exportación con alcance explícito.** Dos rutas nuevas de solo lectura
  (`04 §5.8` en commit previo `c9181a6`): `GET /admin/campanias/{id}/exportar` —`ideas|aportes|
  evaluaciones` en `xlsx|csv`, con `anonimizado` y **los mismos filtros de la pantalla**— y
  `documentos.zip` con nombres legibles. El archivo **declara su alcance** (campaña, filtros, orden,
  total, fecha y quién exportó) en su primera hoja o en líneas `#` del CSV: sin eso, un archivo suelto
  en un correo no se puede auditar. `csv` en UTF-8 con BOM; **tope de 10.000 filas** verificado dos
  veces (ideas y filas reales) que responde `400` en vez de un archivo inservible; `xlsx` y ZIP se
  arman en archivo temporal —sus librerías solo escriben síncrono— y se envían asíncronos con
  `DeleteOnClose`. Para que archivo y pantalla no diverjan, **el listado se refactorizó** a un único
  `ConsultaResultadosCompartida`. Portal: «Exportar» baja el listado visible en Excel y el menú tiene
  los demás recursos, el CSV, el anonimizado (D1) y el ZIP. Backend **1088 unitarias + 141 de
  integración**; portal **87 pruebas en 11 archivos**; build, format, diff, `ng build` producción y
  Prettier verdes. **Siguiente: corte 6/6** (resumen de campaña).
- Actualizacion anterior: 2026-08-21 (Claude Opus 5, Frontend senior/UX-A11Y/SDET): **`P-34` CORTE 4/6
  — DONE LOCAL.** **Solo portal.** Entra la **vista tabla** como principal —participante, área,
  pregunta, extracto, estado, calificación, fechas y documento, con selector de columnas, densidad,
  agrupación por participante colapsable y selección múltiple prevista sin acción (D4)— y el
  maestro-detalle de P-23 se conserva como **vista lectura**, con la elección recordada en sesión. El
  **orden lo resuelve el servidor** (`orden`/`dir` del corte 3), se anuncia con `aria-sort` y al tercer
  clic vuelve al orden natural. Paginación honesta: «Mostrando 1–25 de 30 filtradas · 120 en la
  campaña». La **ficha** muestra por fin la metadata que la API ya devolvía (H-05) —código, empresa y
  sede, `ideaIndice`, fechas absolutas en `America/Bogota` más relativas, confirmación con número de
  versión, rúbrica y modelo, id técnico copiable— y aportes y versiones se intercalan en una **línea de
  tiempo** única. Portal **86 pruebas en 11 archivos**; backend sin cambios (**1073 + 131**); build,
  format, diff, `ng build` producción y Prettier verdes. **Siguiente: corte 5/6** (exportación).
- Actualizacion anterior: 2026-08-21 (Claude Opus 5, Arquitecto/Backend/Frontend/SDET): **`P-34` CORTE
...
- Actualizacion anterior: 2026-08-20 (Claude Opus 5, Arquitecto/Backend senior/SDET): **`P-34` CORTE
  ...
  **pagina antes** de resolver versiones, y las de la página se piden en **una sola consulta por ids**
  ...
  completa de respuestas. Medido con 1.000 ideas y 5.000 aportes sembrados: listado de **1.000
  lecturas puntuales a 0** (1 consulta de ideas + 1 de versiones, 403 → 332 ms) y detalle de **5.000
  documentos de respuesta leídos a 5** (701 → 496 ms). Con el listado ya barato, el portal recorre
  también las páginas de ideas, así que el desglose por estado de H-04 es exacto. Backend **1056
  unitarias + 125 de integración**, build Release `-warnaserror`, `dotnet format` y `git diff --check`
  verdes; portal **77 pruebas en 10 archivos**, `ng build` producción y Prettier verdes. Sin cambios
  de contrato ni de datos. Medición de RU/latencia contra Cosmos real: puerta operativa pendiente.
  **Siguiente: corte 3/6** (identidad y filtros en el servidor).
- Actualizacion anterior: 2026-08-20 (Claude Opus 5, Frontend senior/SDET): **`P-34` CORTE 1/6 — DONE
...
- Ultima actualizacion: 2026-08-15 (Codex, Arquitecto/SDET/AppSec):
  **`DT-I20-02` desplegada; QAS/21 delega la preparación remota al agente de pruebas.** Por
  ...
  para probar gobierno runtime, una campaña aislada duplicada sin participantes y usuarios `es/en`.
  `QAS/21` ahora puntúa borrador, prevalidación, aprobación, asociación en campaña **y pregunta**,
  pruebas 1–8, D5, rollback y evidencia. El alcance excluye familia `1`, campañas reales, rúbricas,
  ConfigLLM, secretos, App Settings, código y despliegues; una migración real exige nueva
  autorización. Siguiente: ejecutar estrictamente `QAS/21_*` y guardar
  `QAS/resultados/Resultados_DT-I20-02_<fecha>.md`.
- Ultima actualizacion: 2026-08-15 (Claude Opus 5, Arquitecto/Backend/SDET/AppSec):
  **`DT-I20-02` COMPLETA LOCAL 3/3 — corte 3/3: prompt candidato, integración de §7.2 y cierre
  documental.** El **contenido candidato** del prompt de evaluación quedó redactado en
  `planes/DT-I20-02_Prompt_Candidato_Evaluacion.md`: texto listo para copiar, explicación de lo que el
  backend ya inyecta —esquema JSON, idioma P-32, anti-inyección, pista de eje débil I-03, variación
  DT-I20-01— para no duplicarlo, mapa de cómo corrige cada causa del reporte y lista de verificación
  humana. **No se creó ni activó nada en Cosmos**: crear la familia **nueva**, aprobarla, asociarla a
  la campaña aislada y migrar campañas sigue siendo acción humana autorizada del runbook. Las pruebas
  de §7.2 corren ahora sobre el **evaluador real** con un `ILlmClient` falso dentro del recorrido
  webhook → cola → orquestador → gateway: el caso reportado llega a WhatsApp sin `###`, sin
  `Lo que ya queda claro` / `Lo que todavía falta` / `Siguiente ajuste recomendado`, sin
  `ready_to_save` y con una sola pregunta; el aporte del participante conserva `caja #3` carácter por
  carácter; y la misma corrida con campos visibles válidos produce **idéntica decisión de negocio**
  (puntajes, calificación por criterio, recomendación, repregunta, estado del hilo, presupuesto de
  repreguntas y trazabilidad idea/versión), cambiando solo el fragmento visible. Backend **925
  unitarias + 112 de integración**; regresiones por iniciativa verdes (I-18 8, I-19 22+1, I-20 20+3,
  DT-I20-01 6, P-27 18+2, P-32 16, P-33 4, DT-I20-02 9+2). Documentación cerrada en `05 §4.4.2`,
  `08 §3.4`, `Reglas_Conversacion_y_Participacion.md §2.14`, la spec, el runbook y `QAS/21`. Build
  Release `-warnaserror`, `dotnet format --verify-no-changes` y `git diff --check` verdes. Sin
  frontend, push, despliegue, Cosmos ni configuración remota. **Pendiente explícito por costo,
  credenciales y autorización humana:** `QAS/21` real y D5 en ambiente aislado. **Siguiente:
  decisión del usuario** — desplegar `DT-I20-02` con autorización y ejecutar QAS/21, o retomar P-32
  completa (QAS/17 y lote mixto real); `DT-P32-04` sigue como backlog post-green.
- Actualizacion anterior: 2026-08-15 (Claude Opus 5, Arquitecto/Backend/SDET):
...
- Actualizacion anterior: 2026-08-15 (Claude Opus 5, Arquitecto/Backend/SDET/AppSec):
  **`DT-I20-02` CORTE 1/3 DONE LOCAL — contrato visible en texto plano con respaldo por campo.**
  ...
  `cantidad_preguntas`, `longitud`) sin devolver ni registrar el texto. Detecta estructura editorial
  **anclada al inicio de línea** —encabezado ATX, viñeta, lista numerada, cita, separador, tabla y
  cerca de código—, de modo que `caja #3`, un guion intercalado o un salto de línea sin estructura
  siguen siendo válidos; las etiquetas del contrato JSON y las órdenes de proceso (`ready_to_save`,
  `save now`, `listo para guardar`) se buscan en cualquier posición, y los títulos ambiguos (`Estado`,
  `Pregunta clave`, `Lo que ya queda claro`, `Resumen` y sus equivalentes en inglés) solo cuentan como
  etiqueta cuando ocupan la línea completa o la abren con dos puntos. En el evaluador,
  `AplicarContratoVisibleAsync` corre después del filtro de fuga de rúbrica y antes de construir la
  evaluación: la retroalimentación inválida cae a `RetroNeutra` y la repregunta inválida a
  `RepreguntaNeutra` **campo por campo**, dejando intactos puntajes, recomendación, idea/versionId,
  madurez, estados, cierre y presupuesto de repreguntas; la auditoría registra
  `contrato_visible` con `componente=evaluador;retroalimentacion=<motivo>;repregunta=<motivo>` y
  nunca el texto. Desapareció el corte ciego a 600 caracteres: ahora se recorta en **frontera de
  oración** y, sin frontera dentro del máximo, se usa el respaldo, así que no se persiste una palabra
  partida. En I-20, `GuardasRedaccionTurno.Rechazar` cierra con el mismo contrato **antes** de que el
  orquestador aplique `FiltroDuplicacionTurno` (DT-I20-01), que sigue omitiendo el puente duplicado
  igual que antes. No se sanitiza el mensaje final, no se toca `VersionIdeaConsolidada` (P-33), ni
  catálogo P-32, mensajes de campaña, plantillas Meta o artefactos Markdown. Backend **912 unitarias
  + 110 de integración** (49 nuevas: validador puro, evaluador e I-20); build Release `-warnaserror`,
  `dotnet format --verify-no-changes` y `git diff --check` verdes. Sin frontend, sin push, sin
  despliegue, sin Cosmos y sin configuración remota. **Siguiente código: `DT-I20-02` corte 2/3**
  (selección runtime de la versión de prompt activa **y** aprobada más nueva, conservando la consulta
  administrativa actual). Las pruebas de integración conversacional de §7.2 y la migración de prompt
  quedan en el corte 3/3.
- Actualizacion anterior: 2026-08-15 (Codex, cierre documental con evidencia Claude/operador):
  **`DT-P32-03-01` CERRADA — IMPLEMENTADA, DESPLEGADA Y P-32 SMOKE GREEN (1/1).** Commit
  `60b520d` está en `main/origin` y desplegado en Azure. La revalidación acotada dejó QAS/23 pruebas
  4–6 PASS; con los PASS conservados de 1–3, las seis pruebas están verdes y la evidencia humana de
  Meta fue aceptada. El gate terminó OFF, el operador apagó `Simulacion__Habilitada` y retiró
  `GHT_DIAG_KEY`. El resultado está en
  `QAS/resultados/Resultados_P32_Smoke_DT-P32-03-01_2026-08-15.md`. **P-32 no se cierra todavía:**
  QAS/17 completa y la prueba 7 del lote mixto real se ejecutan después de DT-I20-02. **Siguiente
  código: DT-I20-02 corte 1/3; DT-P32-04 permanece post-green y no bloquea.**
- Actualizacion anterior: 2026-08-15 (Claude Opus 5, Arquitecto/Backend/Frontend/SDET/AppSec):
  **`DT-P32-03-01` COMPLETA LOCAL 1/1 — readiness del gate limitado a campañas activas.**
  `mapeosMeta[]` conserva las campañas `activa|borrador` y sus problemas, pero agrega
  `bloqueaGateOn` (`true` solo si alguna campaña **activa** exige el par) y `listoParaGateOn` se
  calcula únicamente con esos pares: un borrador incompleto —estado normal de trabajo y sin
  transición a `archivada`— ya no mantiene la señal apagada para siempre. Para que la señal no se
  vuelva mentira, la transición `borrador → activa` valida **con gate ON** y **solo sobre la campaña
  objetivo** los pares que exigen sus mensajes iniciales activos, con la misma política del envío;
  falla con `400 VALIDATION_ERROR`, detalle `mapeosMeta.{mensajeInicialId}.{idioma}` y el estado queda
  en `borrador`. Con gate OFF la activación no cambia. El panel **Preparación** distingue el bloqueo
  de hoy del pendiente antes de activar un borrador. Sin Graph API, nombres físicos, secretos ni
  escritura de App Settings. Backend **863 unitarias + 109 de integración**, portal **62**; build
  Release `-warnaserror`, `dotnet format`, `ng test`, `ng build`, Prettier y `git diff --check`
  verdes. **Sin push, despliegue ni configuración remota; el gate sigue OFF.** **Siguiente histórico:**
  desplegar con autorización y repetir **solo** `QAS/23` pruebas 4–6; con green iniciar `DT-I20-02`
  corte 1/3.
- Ultima actualizacion: 2026-08-15 (Codex, Arquitecto/Backend/Frontend/SDET/AppSec):
  **`DT-P32-03-01` ESPECIFICADA 0/1; sin código ni cambio remoto.** El smoke desplegado de
  DT-P32-03 dejó pruebas 1–4 y 6 PASS y la 5 BLOCKED. El defecto bilingüe está cerrado; el bloqueo
  restante es semántico: campañas borrador incompletas, que son trabajo normal y no pueden
  archivarse, mantienen `listoParaGateOn=false`. El microajuste conserva activas/borradores visibles,
  agrega `mapeosMeta[].bloqueaGateOn`, calcula la señal solo con pares consumidos por campañas activas
  y, con gate ON, valida los mapeos propios antes de permitir `borrador → activa`. No amplía readiness
  para exponer nombres físicos ni consulta Meta: nombre, código, componentes y aprobación se aceptan
  como evidencia humana. Plan de un corte, contratos y QAS/23 actualizados. **Siguiente código:
  implementar DT-P32-03-01 1/1; luego desplegar con autorización, repetir QAS/23 4–6 y, si queda
  green, iniciar DT-I20-02 corte 1/3. DT-P32-04 permanece post-green y no bloqueante.**
- Ultima actualizacion: 2026-08-15 (Claude Opus 5, Arquitecto/Backend/Frontend/SDET/AppSec):
  **`DT-P32-03` COMPLETA Y DESPLEGADA (2/2) — readiness de plantillas Meta y Preparación con dos
  comprobaciones separadas.** `GET
  /api/admin/catalogos-textos/readiness` agrega, de forma aditiva, `listoParaGateOn` y `mapeosMeta[]`:
  `ValidadorMapeosPlantillaMeta` (Application, puro) enumera los pares `plantillaRef + idioma` que
  exigirían las campañas `activa|borrador` con sus mensajes iniciales activos, deduplicados por alias
  + idioma y con las campañas que los piden; el veredicto de "configurado" es el mismo
  `OpcionesPlantillaEnvioInicial.TryResolver` que usa `ServicioEnvios`, así que no hay una segunda
  interpretación. Reporta `plantilla_ref_faltante`, `nombre_faltante`, `idioma_meta_faltante`,
  `componente_vacio` y `componente_duplicado`; una plantilla legítima sin variables puede declarar
  `componentes: []` y quedar lista. `idiomas[].listo` **no cambia** de significado: `listoParaGateOn`
  es la señal agregada (catálogos válidos **y** mapeos configurados). No consulta Graph API ni
  secretos, y por tanto no certifica la aprobación en Meta: eso sigue siendo verificación manual
  (`QAS/23`). El portal muestra «Plantillas de WhatsApp» dentro de **Preparación**, con el problema
  en lenguaje de administrador, quién pide cada plantilla y un veredicto final que distingue
  «los textos están listos» de «listo para empezar a usarlos». Backend **959: 854 unitarias + 105 de
  integración** (15 nuevas: 10 del validador, 3 de readiness y 2 de integración de la ruta) y portal
  **60** (3 nuevas); build Release `-warnaserror`, `dotnet format --verify-no-changes`, `ng test`,
  `ng build`, `prettier --check` de los archivos tocados y `git diff --check` verdes.
  **Desplegado el 2026-08-15:** `main` y `origin/main` apuntan a `a9f4a6f`; el push de los tres
  commits (`63f819c`, `50fee37`, `a9f4a6f`) disparó CI y Deploy, ambos en success. El despliegue **no
  activa nada**: `Conversacion:CatalogoTextosHabilitado` no tiene override en Azure y conserva el
  default OFF, así que el cierre localizado solo cambia comportamiento cuando alguien encienda el
  gate, y la ruta de readiness es aditiva y de solo lectura. **Siguiente: ejecutar `QAS/23` (pruebas 1
  a 7) y luego `QAS/17` completo; solo con green se retoma `DT-I20-02`.**
- Actualizacion anterior: 2026-08-14 (Claude Opus 5, Arquitecto/Backend/SDET/AppSec): **`DT-P32-03`
...
- Actualizacion anterior: 2026-08-14 (Claude Opus 5, Arquitecto/Backend/SDET/AppSec): **`DT-P32-02`
  ...
  impide inicializar `es/en`; `CrearDesdeLegacy` conserva la fotografía completa **sin truncar ni
  mezclar con defaults**. El límite de frases por grupo pasa de 30 compilado a
  `Conversacion:CatalogoTextos:MaxFrasesPorGrupo` (default 100, techo 500) y se agrega
  `MaxBytesImportacionJson` (default 256 KiB, techo 1 MiB); ambos se ajustan al techo en vez de
  derribar el arranque, y el runtime revalida el catálogo activo con el mismo límite. Nuevo validador
  `Prevalidar(...)` puro —mismas reglas que la escritura, conteos y todos los errores, sin contenido—
  compartido por semilla e importación. Rutas aditivas `POST /semillas/{idioma}/base`,
  `GET /semillas/{idioma}/legacy/preview`, `GET /semillas/{idioma}/legacy/exportar` y
  `POST /semillas/{idioma}/legacy`; `POST /semillas/{idioma}` conserva su semántica P-32. Auditoría
  `crearSemillaBase`/`importarLegacy`/`prevalidarLegacy` con acción, idioma, conteos y límite, nunca
  textos. Backend **898** (804 unitarias + 94 integración), build Release `-warnaserror`,
  `dotnet format` y `git diff --check` verdes. Sin frontend, sin push, sin despliegue, sin activación
  y con el gate `Conversacion:CatalogoTextosHabilitado` intacto en OFF.
  **Siguiente código: corte 2/3 de `DT-P32-02`** (JSON masivo, readiness y campañas bilingües).
...
- [ ] **DEFINIR EL SIGUIENTE OBJETIVO CON EL USUARIO.** `P-34` quedó **completa 6/6** y no hay
  trabajo de código en curso. Candidatos ya identificados, ninguno iniciado: (a) el **robustecimiento
  semántico integral `DT-P33-01`** que sigue en backlog —banco semántico, corrida QAS/25
  abierto/cerrado/mixto, D5 `n=3`, costo/latencia y acta—; (b) las **puertas operativas de la
  convención** (parametrización, usuarios reales, D5, smoke/UAT bilingüe, costo/latencia, readiness y
  acta de flags), que no son código; (c) la deuda de **P-34 anotada en su cierre**: columnas
  «versiones»/«aportes» de la tabla y filtros «con/sin documento» y «con/sin evaluación», ambas con
  cambio aditivo de `04 §5.8`. **No empezar ninguna sin decisión explícita del usuario.** Siguen sin
  publicar los commits locales de P-34: el push a `main` despliega y es decisión suya.
- [x] **(HECHO 2026-08-21, Claude Opus 5 — backend 1095 unitarias + 145 integración; portal 89 pruebas
  en 11 archivos; build, format, diff, `ng build` producción y Prettier verdes) `P-34` corte 6/6 —
  COMPLETA.** `/resumen` con los mismos filtros (participación sobre la convocatoria, embudo
  acumulativo, histograma sobre la escala de la rúbrica, cobertura por pregunta y temas estables),
  panel plegado con las cinco cifras visibles y una tabla equivalente por gráfico, y marca de umbral
  solo cuando es uniforme. Contrato en commit previo `267c0b7`. Supuestos en
  `SUPUESTOS.md#resumen-campania-p-34` y `#cierre-p-34`.
- [x] **(HECHO 2026-08-21, Claude Opus 5 — backend 1088 unitarias + 141 integración; portal 87 pruebas
  en 11 archivos; build, format, diff, `ng build` producción y Prettier verdes) `P-34` corte 5/6.**
  `/exportar` (ideas/aportes/evaluaciones en xlsx/csv, anonimizado y filtros de la pantalla) y
  `documentos.zip` con nombres legibles; hoja «Filtros aplicados» dentro del archivo; tope de 10.000
  filas con `400`; escritura sin sincronía sobre el socket ni archivo completo en memoria; listado y
  exportación unificados en `ConsultaResultadosCompartida`. Contrato en commit previo `c9181a6`.
  Supuesto en `SUPUESTOS.md#exportacion-resultados-p-34`.
- [x] **(HECHO 2026-08-21, Claude Opus 5 — portal 86 pruebas en 11 archivos; backend sin cambios
  1073 + 131; build, format, diff, `ng build` producción y Prettier verdes) `P-34` corte 4/6.** Vista
  tabla ordenable como principal (orden resuelto en el servidor, `aria-sort`, tercer clic al orden
  natural), agrupación por participante, selector de columnas, densidad, tamaño de página y
  paginación honesta; maestro-detalle de P-23 conservado como vista lectura y recordado en sesión;
  ficha con toda la metadata (H-05) y línea de tiempo única de aportes y versiones. Supuesto en
  `SUPUESTOS.md#tabla-y-ficha-resultados-p-34`.
...
- [x] **(HECHO 2026-08-20, Claude Opus 5 — backend 1056 unitarias + 125 integración; build Release
  ...
  en una sola consulta por ids (`ListarVersionesDeCampaniaAsync`), aportes por `ideaId`
  (`ListarRespuestasPorIdeaAsync`), degradación por defecto en el puerto y medición 1.000 ideas:
  1.000 → 0 lecturas puntuales y 5.000 → 5 documentos de respuesta. Portal: `ideasTodas` recorre el
  listado completo y vuelve exacto el desglose por estado. Supuesto en
  `SUPUESTOS.md#escala-listado-ideas-p-34`. Pendiente operativo: RU/latencia reales contra Cosmos.
...
- [x] **(HECHO 2026-08-15, Claude Opus 5 — backend 925 unitarias + 112 integración; build Release
  ...
  → gateway, regresiones por iniciativa verdes y cierre documental de `05 §4.4.2`, `08 §3.4` y
  `Reglas_Conversacion_y_Participacion.md §2.14`. D5 y QAS reales quedan pendientes por costo,
...
- [x] **(HECHO 2026-08-15, Claude Opus 5 — backend 912 unitarias + 110 integración; build Release
  ...
  evaluador (`RetroNeutra`/`RepreguntaNeutra`) sin tocar puntajes ni decisiones, guarda de I-20 previa
  a `DT-I20-01` y recorte en frontera de oración en lugar del corte ciego. Sin sanitización global,
  sin cambios de contrato, Cosmos ni configuración remota.
- [x] **DT-P32-03-01 desplegada y smoke green.** Commit `60b520d`; QAS/23 1–6 PASS, evidencia Meta
  aceptada, gate y simulación OFF y clave diagnóstica retirada. P-32 completa queda para después de
  DT-I20-02.
- [x] **(HECHO 2026-08-15, Claude Opus 5 — backend 863 unitarias + 109 integración; portal 62; build
  Release `-warnaserror`, format, `ng test`, `ng build`, Prettier y diff verdes) `DT-P32-03-01` corte
  único 1/1.** `bloqueaGateOn` en `mapeosMeta[]`, agregado limitado a pares de campañas activas,
  guarda de mapeos propios en `borrador → activa` con gate ON y panel **Preparación** que separa el
  bloqueo actual del pendiente de borrador. Sin Graph API, nombres físicos, Azure ni Meta.
- [x] **`DT-P32-03` corte 2/2 desplegado y smoke ejecutado.** QAS/23 pruebas 1–4 y 6 PASS; la 5
  descubrió la semántica corregida por DT-P32-03-01. No reabrir el resolutor de cierre.
...
- [x] **(HECHO 2026-08-14, Claude Opus 5 — backend 898: 804 unitarias + 94 integración; build
  ...
  techo compilado, prevalidación pura compartida y las cuatro rutas de semilla base/legacy. La ruta
  P-32 `POST /semillas/{idioma}` conserva su semántica; el gate sigue OFF y no hubo frontend.
- [x] **P-32 smoke requerido completado.** QAS/22 y QAS/23 1–6 están verdes; QAS/17 completa queda
  programada después de DT-I20-02 por el orden acordado.
- [x] **`DT-I20-02` corte 1/3 ejecutado (2026-08-15).** Su spec, QAS/21 y runbook siguen vigentes; no
  se crearon ni activaron prompts remotos. `QAS/21` sigue pendiente de ejecución humana: sus pruebas
  1–7 pueden correrse ya en ambiente aislado autorizado, y la 8 requiere el corte 2/3.
...
| P-34 | Resultados: identidad, filtros, tabla, exportación y resumen | **COMPLETA 6/6 local — 2026-08-21** | cambios locales + `98b8bca` (contrato) | portal 76 pruebas en 10 archivos, `ng build` producción y Prettier verdes; backend sin cambios (1053 + 122) | Corte 1: paginación completa de usuarios/Markdown/respuestas/conversaciones hasta el `total` del servidor, contadores tomados de `total` con aviso cuando el desglose por estado es parcial, y fallo del maestro de participantes visible y reintentable con «Participante no identificado · código» en vez del id técnico. Sin contratos ni datos; P-23 intacto. Corte 2: el listado pagina **antes** de resolver versiones y pide las de la página en una sola consulta por ids; el detalle trae los aportes por `ideaId`. Medido con 1.000 ideas: 1.000 → **0** lecturas puntuales y 5.000 → **5** documentos de respuesta. Corte 3: `participante` embebido y calificación vigente en `/admin/ideas`, filtros de servidor (`q`, área, empresa, sede, fechas, calificación, confirmada) y `orden`/`dir` con `400` ante consulta inválida; portal con barra de dos niveles, chips, filtro en la URL y vacío que nombra el filtro; `04 §5.8` en commit previo `98b8bca`. Corte 4 (solo portal): vista tabla ordenable como principal con orden resuelto en el servidor y anunciado con `aria-sort`, agrupación por participante, columnas/densidad/tamaño de página configurables, paginación honesta, vista lectura conservada, ficha con toda la metadata (H-05) y línea de tiempo única. Corte 5: `/exportar` (ideas/aportes/evaluaciones en xlsx/csv, con anonimizado y los filtros de la pantalla) y `documentos.zip`, con «Filtros aplicados» dentro del archivo, tope de 10.000 filas y escritura sin sincronía sobre el socket; `04 §5.8` en commit previo `c9181a6`. Corte 6: `/resumen` con los mismos filtros —participación, embudo, histograma, cobertura y temas—, panel plegado con las cinco cifras visibles y tabla equivalente por gráfico; `04 §5.8` en commit previo `267c0b7`. Supuestos `SUPUESTOS.md#paginacion-y-contadores-resultados-p-34`, `#escala-listado-ideas-p-34`, `#identidad-y-filtros-servidor-p-34`, `#tabla-y-ficha-resultados-p-34`, `#exportacion-resultados-p-34`, `#resumen-campania-p-34` y `#cierre-p-34`. **Cómo probarlo:** abre Resultados con una campaña que tenga más de 25 documentos y comprueba que «Descargar .md» aparece también en las ideas del final de la lista, que el número de ideas del encabezado coincide con el de la campaña y que, si la lista de participantes falla, la pantalla lo dice y ofrece «Reintentar». Siguiente: corte 2/6 (escala a 1.000 ideas, backend). |
...
- 2026-08-21 - Claude Opus 5 - **`P-34` corte 6/6 — resumen de campaña; la iniciativa queda
  COMPLETA.** Rol: Backend senior .NET para el endpoint, Frontend + UX/A11Y para el panel, SDET para
  las regresiones. Cubre `P-34 §4.6`, `§3.1` y `§8.9`; contrato `04 §5.8` en **commit aparte y previo**
  (`267c0b7`), `03` sin cambios. **Qué entra:** `GET /admin/campanias/{id}/resumen` con los mismos
  filtros del listado —participación sobre la convocatoria completa, embudo acumulativo, distribución
  de calificaciones, cobertura por pregunta y temas—, resuelto en el servidor porque calcularlo en el
  navegador obligaría a descargar las 1.000 ideas (D5). Pasa por el mismo
  `ConsultaResultadosCompartida` que la tabla y la exportación, así que **`totalIdeas` coincide
  siempre con el `total` del listado** (§8.9): es la propiedad que hace confiable el panel.
  `ResumenCampaniaResultados` (Application, puro) calcula mediana, tramos y conteos sin tocar E/S.
  **Decisiones que no son obvias:** `convocados` es la campaña completa —el denominador de «¿cuánta
  gente participó?»— mientras `conIdeas` sale del filtro; los tramos cubren la escala declarada por el
  snapshot de rúbrica y el último incluye su extremo para no perder la nota máxima, y sin snapshot se
  usan los valores observados en vez de inventar un rango; `temas` desempata alfabéticamente para que
  dos llamadas iguales devuelvan lo mismo; y la **marca del umbral solo se dibuja cuando es uniforme**
  —ninguna pregunta lo sobrescribe y todas las evaluaciones comparten escala—, porque una línea que no
  vale para parte del histograma es peor que ninguna. **Portal:** panel **plegado por defecto** con
  las cinco cifras en el encabezado (§3.1) y, dentro, una **tabla con barra CSS** por cada gráfico:
  sin librerías nuevas (`01 §11`), con tooltip del valor exacto y el dato legible sin ver el color
  (P-18/P-19). **Pruebas:** 7 unitarias del cálculo puro y 4 de integración —incluida la de §8.9, que
  compara el `total` del listado con el `totalIdeas` del resumen para el mismo filtro— más 2 de
  componente. Backend **1095 unitarias + 145 de integración**; portal **89 pruebas en 11 archivos**;
  build Release `-warnaserror`, `dotnet format`, `git diff --check`, `ng build --configuration
  production` y Prettier verdes. Decisiones en `SUPUESTOS.md#resumen-campania-p-34` y `#cierre-p-34`,
  que además deja anotado lo que quedó deliberadamente fuera de P-34 y la puerta operativa abierta
  (RU/latencia reales contra Cosmos). **P-34 queda completa 6/6**; sin push ni despliegue.
- 2026-08-21 - Claude Opus 5 - **`P-34` corte 5/6 — exportación con alcance explícito.** Rol:
  Arquitecto para la frontera del puerto de escritura, Backend senior .NET para endpoints y adaptador,
  Frontend para el menú de descarga, SDET para las regresiones. Cubre `P-34 §4.5`, `§5` y `§7`;
  contrato `04 §5.8` en **commit aparte y previo** (`c9181a6`), `03` sin cambios. **Qué entra:**
  `GET /admin/campanias/{id}/exportar` —`recurso` = ideas|aportes|evaluaciones, `formato` = xlsx|csv,
  `anonimizado`, más los mismos filtros del listado— y `GET /admin/campanias/{id}/documentos.zip`, un
  `.md` por idea con nombre `U-000042_Ana-Perez_idea-1.md`. **El alcance lo resuelve el servidor** y
  se ignoran `page`/`pageSize`: exportar media página sería una trampa. Para que el archivo y la
  pantalla no puedan divergir, ambos pasan por el nuevo `ConsultaResultadosCompartida` y **el listado
  se refactorizó** para usarlo en vez de su copia del filtro. **El archivo declara su alcance**
  —campaña, cada filtro, orden, total, fecha y quién exportó— como primera hoja en `xlsx` y como
  líneas `#` en `csv`; sin eso, un archivo suelto en un correo no se puede auditar tres semanas
  después. `csv` en UTF-8 **con BOM**, porque sin él Excel abre los acentos rotos; nombres de archivo
  en ASCII, porque los acentos viajan mal en `Content-Disposition`. **Tope de 10.000 filas** verificado
  dos veces —sobre las ideas del alcance y sobre las filas reales, ya que un recurso de grano fino
  multiplica lo que en ideas cabía— con `400` y el total encontrado. **Cómo se escribe:** el `csv` va
  fila por fila sobre la respuesta; ClosedXML y `ZipArchive` solo escriben síncrono y hacerlo sobre el
  socket está prohibido (`AllowSynchronousIO`) además de bloquear un hilo, así que se arman en un
  archivo temporal —una entrada por vez— y se envían asíncronos con `DeleteOnClose`, que lo borra
  aunque el cliente corte. El contrato se corrigió en este commit: la versión documental suponía que
  el `xlsx` se armaba en memoria. **Anonimizado (D1):** sustituye el nombre por el código legible en
  todas las columnas y en los nombres del ZIP; área, empresa y sede se conservan porque no identifican
  por sí solas. Sin dependencias nuevas: ClosedXML ya estaba por la carga masiva de I-08. **Portal:**
  «Exportar» baja el listado visible en Excel y el menú tiene los demás recursos, el CSV, la casilla
  de anonimizado y el ZIP; el archivo se guarda con el nombre que manda el servidor. **Pruebas:** 15
  unitarias del constructor puro y 10 de integración (BOM y hoja de filtros del CSV, dos hojas del
  xlsx, filtros respetados y declarados, anonimizado sin rastro del nombre, aportes con su versión,
  evaluaciones con criterios y modelo, `400` por parámetro inválido, ZIP con nombres legibles y solo
  lo filtrado, y autorización: anónimo `401`, visor `200`), más una de componente en el portal.
  Backend **1088 unitarias + 141 de integración**; portal **87 pruebas en 11 archivos**; build Release
  `-warnaserror`, `dotnet format`, `git diff --check`, `ng build --configuration production` y
  Prettier verdes. Decisión en `SUPUESTOS.md#exportacion-resultados-p-34`. Sin push ni despliegue.
  **Siguiente: corte 6/6** (resumen de campaña), el último de P-34.
- 2026-08-21 - Claude Opus 5 - **`P-34` corte 4/6 — la tabla compara, el maestro-detalle lee (solo
  portal).** Rol: Frontend senior Angular y UX/A11Y para la vista, SDET para las regresiones. Cubre
  `P-34 §4.3` y `§4.4`; sin backend, contratos, datos ni configuración. **Qué cambió:** (1) entra la
  **vista tabla** como principal —selección · participante (nombre + código) · área · pregunta ·
  extracto · estado · calificación · creada · actualizada · documento—, con selector de columnas,
  densidad, tamaño de página, **agrupación por participante** colapsable y **selección múltiple sin
  acción** (D4, para que la curaduría futura sea agregar un botón y no rehacer la tabla); el
  maestro-detalle de P-23 se conserva como **vista lectura** y la elección vive en
  `ResultadosSesionService` (sesión, sin `localStorage`). (2) **El orden lo resuelve el servidor**: el
  encabezado manda `orden`/`dir` —aceptados desde el corte 3— y recarga, porque ordenar en el cliente
  sobre una página sería un orden falso; `aria-sort` anuncia el estado y el tercer clic **vuelve al
  orden natural** de I-19 en vez de dejarlo fijado. (3) **Paginación honesta**: «Mostrando 1–25 de 30
  filtradas · 120 en la campaña», con el total de la campaña pedido aparte con `pageSize=1` y omitido
  si falla. (4) La **ficha** muestra la metadata que la API ya devolvía y no se pintaba (H-05): código,
  empresa y sede, pregunta, `ideaIndice`, estado, creada y actualizada **absolutas en `America/Bogota`
  más el «hace cuánto»**, confirmación con número de versión, curaduría, motivo de cierre, rúbrica y
  modelo del snapshot, e id técnico copiable. (5) `construirLineaTiempo` (puro, con pruebas propias)
  intercala aportes y versiones en **una sola secuencia**, con la confirmación como evento propio en su
  fecha real: mostrarla en el instante de la versión contaría mal la conversación. **Qué NO se hizo:**
  las columnas «versiones» y «aportes» de `§4.3` —sus conteos no están en el DTO de lista y exigirían
  otro cambio aditivo de `04 §5.8`; la ficha sí los lista—. **Pruebas:** 3 nuevas del helper puro y 4
  de componente (tabla por defecto y orden servidor con `aria-sort` en sus tres pasos, paginación y
  su resumen, vista lectura conservada y recordada, ficha con metadata y línea de tiempo); las
  regresiones de P-23, H-01 y H-04 ahora cambian a «vista lectura» primero, que es donde vive ese
  comportamiento. Portal **86 pruebas en 11 archivos**; backend sin cambios verificado igual (**1073
  unitarias + 131 de integración**); build Release `-warnaserror`, `dotnet format`, `git diff --check`,
  `ng build --configuration production` y Prettier verdes. Decisión en
  `SUPUESTOS.md#tabla-y-ficha-resultados-p-34`. Sin push, despliegue ni configuración remota.
  **Siguiente: corte 5/6** (exportación, con `04 §5.8` en commit aparte y previo).
...
- 2026-08-20 - Claude Opus 5 - **`P-34` corte 2/6 — el listado de ideas a escala de 1.000 (H-10).**
  ...
  `§8.4`; sin tocar `03`, `04`, rutas, permisos ni datos. **El problema:** `GET /admin/ideas`
  resolvía la versión vigente **idea por idea y antes de paginar** —una lectura puntual por idea, dos
  cuando la confirmada no existía—, y `GET /admin/ideas/{id}` leía la **partición completa** de
  respuestas para quedarse con los aportes de una sola idea. **Qué cambió:** (1) el listado filtra,
  ordena, **recorta la página** y solo entonces resuelve versiones —el orden `preguntaId → ideaIndice
  → creadaEn` no depende del texto—, con `total` siempre sobre el conjunto filtrado completo;
  (2) `IRepositorioRespuestas` suma `ListarVersionesDeCampaniaAsync(campaniaId, versionIds, ct)`, que
  Cosmos traduce a **una consulta** `ARRAY_CONTAINS` dentro de la partición, y
  `ListarRespuestasPorIdeaAsync`, que consulta por `ideaId`; ambos con **implementación por defecto
  que degrada al comportamiento anterior** (lecturas puntuales y filtro en memoria), así que un
  adaptador o doble sin la consulta nativa devuelve lo mismo, solo que más caro; (3) el portal, con el
  listado ya barato, recorre también las páginas de ideas (`ideasTodas`), que es lo que vuelve exacto
  el desglose por estado de H-04. **Qué NO cambió:** la página devuelta, el orden, el `total`, la
  precedencia confirmada→propuesta —una referencia sin documento sigue cayendo a la propuesta— ni el
  contenido del detalle. **Medición** (`ListadoIdeasEscalaP34IntegrationTests`, 1.000 ideas y 5.000
  aportes sembrados, doble que cuenta operaciones como Cosmos): listado `pageSize=100` pasa de
  **1 consulta + 1.000 lecturas puntuales (403 ms)** a **1 consulta de ideas + 1 de versiones con 100
  ids y 0 lecturas puntuales (332 ms)**; el detalle pasa de **5.000 documentos de respuesta leídos
  (701 ms)** a **5 (496 ms)**. Es una medición de **operaciones**, in-process: las RU y la latencia
  reales contra Cosmos siguen siendo puerta operativa con ambiente y credenciales. **Pruebas:** 3 de
  integración nuevas (listado, última página y detalle) y 3 unitarias del adaptador Cosmos (consulta
  única por ids, sin ids no consulta nada, aportes por `ideaId`); backend **1056 unitarias + 125 de
  integración**, build Release `-warnaserror`, `dotnet format` y `git diff --check` verdes; portal
  **77 pruebas en 10 archivos**, `ng build --configuration production` y Prettier verdes. Decisión en
  `SUPUESTOS.md#escala-listado-ideas-p-34`. Sin push, despliegue, Cosmos ni configuración remota.
  **Siguiente: corte 3/6** (identidad embebida y filtros de servidor, con `04 §5.8` en commit aparte).
...
- 2026-08-14 - Claude Opus 5 - **`DT-P32-03` corte 2/2 — readiness de plantillas Meta y Preparación
  ...
  Preparación, SDET para las pruebas y AppSec para la superficie expuesta. Cubre `DT-P32-03 §3.2`,
  `§4`, `§5`, `§6.5-§6.10` y `§7`; contrato `04 §5.7`, `QAS/23` pruebas 4 a 7.
  **Qué cambió:** (1) `ValidadorMapeosPlantillaMeta` (Application, puro, sin E/S) enumera los pares
  `plantillaRef + idioma` requeridos por las campañas `activa|borrador` y sus mensajes iniciales
  **activos**, deduplicados por alias + idioma y acumulando las campañas que los piden
  (`campaniaId`, nombre, estado, `mensajeInicialId`). El juicio de "configurado" delega en
  `OpcionesPlantillaEnvioInicial.TryResolver` —la misma política del envío real— y encima se reportan
  los problemas estructurales `plantilla_ref_faltante`, `nombre_faltante`, `idioma_meta_faltante`,
  `componente_vacio` y `componente_duplicado`. Un mensaje inicial activo sin alias no desaparece del
  diagnóstico. (2) `ServicioReadinessCatalogosTextos` incorpora `MapeosMeta` y expone
  `ListoParaGateOn` = catálogos por idioma listos **y** todos los mapeos listos; `idiomas[].listo`
  conserva su significado editorial y ningún consumidor existente cambia. (3) El endpoint agrega
  `listoParaGateOn` y `mapeosMeta[]` de forma aditiva. (4) El portal muestra «Plantillas de WhatsApp»
  dentro de **Preparación**: cada par con su problema traducido a lenguaje de administrador, la lista
  de campañas que lo piden, la advertencia explícita de que esto **no** confirma la aprobación en
  Meta, y un veredicto final que separa «los textos están listos» de «listo para empezar a usarlos».
  **Qué NO cambió:** no se consulta Graph API, no se leen ni registran secretos, no se crean ni
  modifican plantillas en Meta, no se escriben App Settings y el portal no se internacionalizó.
  **Pruebas:** 15 nuevas — 10 unitarias del validador (par listo, componentes vacíos legítimos, par
  sin mapeo, código Meta ausente, componente vacío/duplicado, alias faltante, mensaje inactivo,
  idioma fuera de alcance, deduplicación entre campañas y un alias por idioma), 3 unitarias de
  readiness (`listoParaGateOn` falso con catálogo listo y mapeo ausente, verdadero con mapa completo,
  y `plantilla_ref_faltante`) y 2 de integración de `GET /api/admin/catalogos-textos/readiness`
  (falta selectiva de `en` sobre un lote bilingüe con ambos catálogos activos, y mapa completo). El
  portal suma 3 pruebas. Totales: backend **854 unitarias + 105 de integración**, portal **60**;
  build Release `-warnaserror`, `dotnet format --verify-no-changes`, `ng test`, `ng build`,
  `prettier --check` de los archivos tocados y `git diff --check` verdes.
  **Nota de gate:** `npx prettier . --check` sigue reportando 10 archivos preexistentes del andamiaje
  (`.prettierrc`, `angular.json`, `tsconfig*.json`, `README.md`, `.vscode/*`) que este corte no tocó.
  **Despliegue (2026-08-15):** los tres commits (`63f819c`, `50fee37`, `a9f4a6f`) se empujaron a
  `origin/main`; CI y Deploy terminaron en success y el artefacto quedó en el App Service. No se tocó
  configuración de Azure: `Conversacion:CatalogoTextosHabilitado` sigue sin override y por tanto OFF,
  de modo que el cierre localizado no cambia el comportamiento vigente y `mapeosMeta[]` solo agrega
  diagnóstico de solo lectura.
  **Cierre posterior:** DT-P32-03-01 se desplegó en `60b520d`; QAS/23 1–6 PASS y P-32 smoke green.
  QAS/17 completa queda para después de DT-I20-02.
  **Cómo probarlo (lenguaje humano):** (1) entra al portal, abre **Textos de conversación** y mira el
  bloque **Preparación**; (2) además del estado por idioma debe aparecer «Plantillas de WhatsApp» con
  una línea por plantilla que las campañas necesiten, diciendo si está configurada y qué campaña la
  pide; (3) si a una campaña bilingüe le falta el mapeo en inglés, esa línea debe decir que falta el
  nombre y el código de idioma de Meta, y abajo debe leerse que los textos están listos **pero**
  todavía no se puede empezar; (4) cuando ambos idiomas estén configurados, el mensaje final debe
  decir que todo lo que se revisa ahí está listo; (5) recuerda que esta pantalla no puede confirmar
  que Meta aprobó la plantilla: eso se revisa a mano en el administrador de WhatsApp. Guía completa:
  `QAS/23`, pruebas 4 a 7.
...
- 2026-08-14 - Claude Opus 5 - **`DT-P32-02` corte 1/3 — semilla base separada del legacy, límites
  ...
  Cubre `DT-P32-02 §2.1/§2.4/§3.3/§4/§7/§8` (corte 1), `04 §5.7.1`, `07 §5.3` y `REQ §19`.
  **Qué cambió:** (1) `CatalogosTextosSemilla` expone `CrearBase(idioma)` —contenido curado compilado,
  sin leer `OpcionesConversacion`— y `CrearDesdeLegacy(idioma, opciones)` —fotografía de
  `Conversacion:Mensajes:*`/`Conversacion:Frases*` efectivos, completa y sin truncar—; `CrearSolicitud`
  se conserva y ahora delega, de modo que `POST /semillas/{idioma}` y `ResolutorTextosConversacion`
  mantienen su comportamiento P-32 exacto, y el respaldo de emergencia usa la base. (2) Nueva
  `PoliticaLimitesCatalogoTextos` (100 frases por grupo con techo 500; 256 KiB de JSON con techo
  1 MiB; 200/1000 caracteres sin cambio) alimentada desde
  `Conversacion:CatalogoTextos:MaxFrasesPorGrupo`/`MaxBytesImportacionJson`, con *clamp* al techo en
  vez de fallar el arranque. (3) `ValidadorCatalogoTextosConversacion` recibe esa política y agrega
  `Prevalidar(...)`, que devuelve `valido`, conteos y **todos** los errores tipificados sin lanzar y
  sin contenido; la escritura real ejecuta exactamente las mismas reglas. (4) Cuatro rutas aditivas de
  `§4` (`/base`, `/legacy/preview`, `/legacy/exportar`, `/legacy`): el preview responde `200` con
  `valido:false` para que el admin corrija, la exportación entrega la fotografía completa aunque sea
  inválida con `formato:catalogo-textos/v1`, y solo una fotografía válida crea borrador. (5) El
  proveedor de runtime revalida el catálogo activo con el límite configurado, no con el compilado.
  (6) Auditoría `crearSemillaBase`/`importarLegacy`/`prevalidarLegacy` con acción, idioma, conteos,
  número de errores y límite efectivo; nunca mensajes, frases ni JSON. **Decisiones registradas** en
  `SUPUESTOS.md#semillas-y-limites-catalogo-dt-p32-02` (inglés sin legacy propio, revalidación runtime
  con el límite configurado, clamp en vez de fallo, y adelanto del `formato` canónico).
  **Verificado local (SDK 8.0.412):** `dotnet build -c Release -warnaserror` 0/0;
  `dotnet test -c Release --no-build --filter "Category!=Calibracion"` **898 verdes (804 unitarias +
  94 de integración; +15 unitarias y +7 de integración)**; `dotnet format --verify-no-changes` y
  `git diff --check` limpios. Sin frontend (corte 3), sin push, sin despliegue y sin tocar el gate.
  **Cómo probarlo (lenguaje simple):** en un ambiente de prueba sin textos cargados, pedir la semilla
  base en español y en inglés y ver que ambas quedan como borrador con todo el contenido, aunque la
  configuración anterior tenga una lista demasiado larga; luego pedir la revisión de la configuración
  anterior y comprobar que el aviso dice qué grupo se pasó del límite y cuántas frases tiene, que se
  puede descargar esa configuración completa para corregirla, y que después de revisarla no apareció
  ninguna versión nueva. Sería un fallo que la semilla base fallara, que alguna lista apareciera
  recortada o que la revisión creara o activara algo. **Siguiente:** corte 2/3 (JSON masivo,
  readiness y campañas bilingües).
...
- 2026-08-15 - Codex - **DT-P32-03-01 readiness solo con campañas activas — ESPECIFICADA 0/1.** El
  smoke de DT-P32-03 cerró el defecto bilingüe y aisló una semántica incorrecta del agregado: los
  borradores incompletos bloqueaban el gate para siempre. Se actualizaron iniciativa, plan, contratos,
  QAS, guía operativa, índices y handoff. Borradores seguirán visibles sin bloquear; con gate ON la
  activación validará los mapeos propios. Solo documentación; sin código, Azure, Meta, despliegue ni
  push. Handoff: implementar el corte, repetir QAS/23 4–6 y después iniciar DT-I20-02.
- 2026-08-15 - Claude Opus 5 - **DT-P32-03-01 corte único 1/1 — COMPLETA local.** `bloqueaGateOn`
  derivado de la presencia de una campaña requirente `activa`, `listoParaGateOn` calculado solo con
  esos pares y guarda de mapeos propios en `borrador → activa` cuando el gate está ON, reutilizando
  `ValidadorMapeosPlantillaMeta` (y por tanto `OpcionesPlantillaEnvioInicial.TryResolver`) para no
  crear una segunda interpretación de "configurado". Panel **Preparación** con bloqueo actual y
  pendiente de borrador separados. Backend 863 unitarias + 109 de integración (9 + 4 nuevas), portal
  62 (2 nuevas); build Release `-warnaserror`, `dotnet format`, `ng test`, `ng build`, Prettier y
  `git diff --check` verdes. Sin push, despliegue, configuración remota ni consultas a Meta; gate OFF.
  Handoff: desplegar con autorización, repetir QAS/23 4–6 y con green iniciar DT-I20-02 corte 1/3.
- 2026-08-15 - Codex - **DT-P32-03-01 CERRADA documentalmente.** Se verificó `60b520d` en
  `main/origin` y desplegado; el reporte Azure dejó QAS/23 4–6 PASS y, junto con 1–3, P-32 smoke
  green. Evidencia Meta aceptada. El operador confirmó gate OFF, simulación OFF y retiro de
  `GHT_DIAG_KEY`. P-32 sigue abierta para QAS/17 completa y lote real después de DT-I20-02. Handoff:
  DT-I20-02 corte 1/3, sin adelantar DT-P32-04.
- 2026-08-15 - Claude Opus 5 - **DT-I20-02 corte 1/3 — DONE local.** Rol: Arquitecto/Backend/SDET/
  AppSec. Nuevo `ValidadorFragmentoVisibleLlm` (Application, puro) con `TipoFragmentoVisible`,
  `ContextoFragmentoVisible` y motivos fijos; detecta estructura Markdown solo al inicio de línea,
  etiquetas internas del contrato en cualquier posición y títulos ambiguos solo como línea o
  encabezado con dos puntos. `EvaluadorLlm.AplicarContratoVisibleAsync` sustituye **por campo** la
  retroalimentación o la repregunta inválida y deja intactos puntajes, recomendación, idea/versionId y
  todas las decisiones server-side; se eliminó el corte ciego a 600 caracteres a favor de un recorte
  en frontera de oración con respaldo. `GuardasRedaccionTurno.Rechazar` aplica el mismo contrato a los
  fragmentos de I-20 antes de `FiltroDuplicacionTurno`. Auditoría `contrato_visible` con motivos de
  baja cardinalidad y sin texto. Verificado: build Release `-warnaserror`, **912 unitarias + 110 de
  integración**, `dotnet format --verify-no-changes` y `git diff --check` verdes. Sin frontend, push,
  despliegue, Cosmos ni configuración remota. Supuesto en
...
- 2026-08-15 - Claude Opus 5 - **DT-I20-02 corte 3/3 — COMPLETA local (3/3).** Rol: Arquitecto/
  Backend/SDET/AppSec. Contenido candidato del prompt de evaluación redactado en
  `planes/DT-I20-02_Prompt_Candidato_Evaluacion.md` **sin crear ni activar nada en Cosmos**, con lo que
  el backend ya inyecta para no duplicarlo y la lista de verificación humana previa; el runbook apunta
  a él. Pruebas de §7.2 sobre el **evaluador real** con `ILlmClient` falso en el recorrido webhook →
  cola → orquestador → gateway: el caso reportado llega sin encabezados, sin etiquetas internas y con
  una sola pregunta; `caja #3` del participante se conserva; y la corrida equivalente con campos
  visibles válidos produce idéntica decisión de negocio. Regresiones por iniciativa verdes (I-18 8,
  I-19 22+1, I-20 20+3, DT-I20-01 6, P-27 18+2, P-32 16, P-33 4, DT-I20-02 9+2). Cierre documental en
  `05 §4.4.2`, `08 §3.4`, `Reglas_Conversacion_y_Participacion.md §2.14`, la spec, el runbook y
  `QAS/21`. Verificado: build Release `-warnaserror`, **925 unitarias + 112 de integración**,
  `dotnet format --verify-no-changes` y `git diff --check` verdes. Sin frontend, push, despliegue,
  Cosmos ni configuración remota. Pendiente explícito: `QAS/21` y D5 reales, por costo, credenciales y
  autorización humana. Handoff: **decisión del usuario** entre desplegar y ejecutar QAS/21 o retomar
  P-32 completa.
- 2026-08-15 - Codex - **Instrucciones QAS/21 actualizadas para preparación autónoma y acotada.**
  El usuario confirmó `DT-I20-02` desplegada y autorizó que el mismo agente de pruebas cree la familia
