...
| **I-08 carga masiva** (I-08 v2) | XLSX/CSV con la plantilla oficial de 9 columnas → reporte por fila; idempotente; `codigoUsuario` secuencial; un solo activo por teléfono con reasignación y conflicto de titular; modo `solo_actualizar`; sin PII en logs | ADM-08, ADM-08a, ADM-08b, ADM-08c, ADM-09, ADM-09a | CORE | **REABIERTA 2026-08-07** — v1 DONE pero la plantilla oficial de GHT tiene otras columnas. Spec y contratos listos, **sin código**. Requiere recrear `users` con unique key `/claveUnicidad`. | ADM-08c necesita Cosmos real para verificar el `409` |
