...
| I-08 carga masiva | **DONE 7/7 y DESPLEGADA (`I-08 v2`, 2026-08-07)**; validada contra Azure el 2026-08-08 | Probar XLSX/CSV con la **plantilla oficial de 9 columnas** + reporte por fila, cabecera inválida, conflicto de titular y reasignación, guardarraíl de un solo activo por teléfono, y modo `solo_actualizar`. **ADM-08c exige Cosmos real** (verifica el `409` de la unique key `/claveUnicidad`). |
| **Listado de evaluaciones (DT-QA-02)** | **DONE local 2026-08-08** | `GET /api/admin/evaluaciones` lista, filtra y diagnostica los cuatro estados sin texto libre; las pruebas automatizadas cubren autorización, PII y el criterio I-16. Pendiente operativo: probar ADM-09b/09c/09d contra el despliegue antes del dry-run. ⚠️ **`superada` no es huérfana**. |
