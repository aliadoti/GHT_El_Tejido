...
| I-08 carga masiva | **REABIERTA 2026-08-07 (`I-08 v2`)** — v1 DONE, plantilla superada | Probar XLSX/CSV con la **plantilla oficial de 9 columnas** + reporte por fila, cabecera inválida, conflicto de titular y reasignación, guardarraíl de un solo activo por teléfono, y modo `solo_actualizar`. **ADM-08c exige Cosmos real** (verifica el `409` de la unique key `/claveUnicidad`). Precondición: contenedor `users` recreado (`I-08 §3.2`). |
